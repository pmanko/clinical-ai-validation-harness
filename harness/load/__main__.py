...
from harness.load.pipeline import run_load


def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="python -m harness.load")
    sub = p.add_subparsers(dest="cmd", required=True)
    run = sub.add_parser("run", help="Run the direct loader (SQLMesh snapshots → build schema).")
    run.add_argument("--target", default="openmrs_test",
                     help="Target schema (default: openmrs_test).")
    args = p.parse_args(argv)

    if args.cmd == "run":
        report = run_load(target_schema=args.target)
