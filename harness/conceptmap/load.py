"""Parse the accepted FHIR R4 ConceptMap.

Returns typed dataclasses that downstream code (seed emit, transform
orchestrator, audits) can consume. Profile shape: ``specs/.../contracts/conceptmap.profile.md``.
The resource shell is validated via ``fhir.resources`` (FHIR R4B);
harness-specific extensions are read as raw dicts since they're our own URLs.
"""
...
class HarnessExtensions:
    ...
    seed_augment_class: str | None = None
