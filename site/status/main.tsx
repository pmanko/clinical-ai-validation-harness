...
function Overview({ project }: { project: Project }) {
  ...
  return <>
    ...
    <div className="scope-note"><strong>Current checkpoint: visual remediation and release evidence.</strong> The four usability iterations are merged. Current visual fixes and runtime verification follow the original delivery roadmap; OpenELIS reporting continues independently. <a href="/clinical-ai-validation-harness/catalyst-design/" target="_blank" rel="noreferrer">Open the frozen design ↗</a> The OpenMRS trio and older contributions remain separate review work. <button className="text-button" onClick={() => openRecord('efforts', 'CAT-E11')}>Open the active iteration →</button></div>
    ...
    <div className="scope-note"><strong>Evidence has a date.</strong> Core delivery and PR records were checked {date(snapshot)}; other entries retain their own evidence dates. It is not a live GitHub feed. Implementation, a passing check, a public demo, and owner acceptance are separate signals. <button className="text-button" onClick={() => navigate('guide')}>How we keep this current →</button></div>
  </>;
}
