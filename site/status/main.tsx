...
function Overview({ project }: { project: Project }) {
  ...
  return <>
    ...
    <div className="scope-note"><strong>Latest Catalyst update: the roadmap and compatible baseline are merged.</strong> Specification consolidation is the active iteration, followed by the complete usability design and a local dual-source owner gate. The OpenMRS trio and older contributions remain separate review work. <button className="text-button" onClick={() => openRecord('efforts', 'CAT-E10')}>Open the active iteration →</button></div>
    ...
    {project === 'all' && <div className="metrics" aria-label="Inventory snapshot across all projects"><button onClick={() => navigate('pull-requests', { project: 'all', state: 'open' })}><strong>{prSnapshot.counts.byState.open}</strong><span>Open pull requests</span><small>Across maintained and upstream work</small></button><button onClick={() => openRecord('efforts', 'CAT-E10')}><strong>1</strong><span>Active Catalyst iteration</span><small>Specification consolidation</small></button><button onClick={() => navigate('decisions', { project: 'all' })}><strong>{decisionCount}</strong><span>Findings to resolve</span><small>Plus one explicitly deferred</small></button><button onClick={() => navigate('reports', { project: 'all' })}><strong>{inventories.reports.rows.length}</strong><span>Published reports</span><small>Dated evidence and its limits</small></button></div>}
