...
import mergeReview from '../../specs/artifacts/project-status/reviews/2026-09-12.md?raw';
...
function Detail({ state }: { state: Location }) {
  ...
  const preferred = ['intent', 'currentSignal', 'mergeReview', 'implemented', 'evidence', 'acceptance', 'next_deliverable', 'nextAction', 'next_action', 'next_update', 'claim_limit', 'catalog_status', 'summary', 'recorded_takeaway', 'current_or_superseded', 'dependencies', 'implementation_owner', 'last_checked', 'last_work', 'source_date', 'source', 'sources', 'url', 'path', 'note'];
...
function Overview({ project }: { project: Project }) {
  ...
  return <>
    ...
    <div className="scope-note"><strong>OpenELIS reporting pathways.</strong> The dated records below track implementation, local checks, server demonstrations and owner acceptance. Native reporting and Catalyst keep their own delivery owners. <a href="/clinical-ai-validation-harness/catalyst-design/" target="_blank" rel="noreferrer">Open the approved design ↗</a> Existing release acceptance and OpenMRS work remain separately tracked. <button className="text-button" onClick={() => openRecord('efforts', 'OE-reporting')}>Open the reporting roadmap →</button></div>
    <div className="project-cards">{cards.filter(c => project === 'all' || project === c.project).map(c => <section className={`project-card ${c.project}`} key={c.project}><div className="card-heading"><h2>{c.title}</h2><span className="project-mark" aria-hidden="true">{c.project === 'catalyst' ? 'C' : c.project === 'chartsearch' ? 'CS' : 'OE'}</span></div><Badge>{c.status}</Badge><p className="card-summary">{c.summary}</p><p className="evidence-note">{c.evidence}</p><div className="card-actions">{c.actions.map(([title, id]) => <button key={id} onClick={() => openRecord('efforts', id)}><span>{title}</span><span aria-hidden="true">→</span></button>)}</div><button className="text-button card-browse" onClick={() => navigate('efforts', { project: c.project as Project })}>Browse {c.title} work →</button></section>)}</div>
    ...
    {project === 'all' && <div className="metrics" aria-label="Inventory snapshot across all projects"><button onClick={() => navigate('pull-requests', { project: 'all', state: 'open' })}><strong>{prSnapshot.counts.byState.open}</strong><span>Open pull requests</span><small>Across maintained and upstream work</small></button><button onClick={() => openRecord('efforts', 'CAT-E11')}><strong>Review</strong><span>Catalyst checkpoint</span><small>Visual fixes and release evidence</small></button><button onClick={() => navigate('decisions', { project: 'all' })}><strong>{decisionCount}</strong><span>Findings to resolve</span><small>Plus one explicitly deferred</small></button><button onClick={() => navigate('reports', { project: 'all' })}><strong>{inventories.reports.rows.length}</strong><span>Published reports</span><small>Dated evidence and its limits</small></button></div>}
    ...
    <div className="scope-note"><strong>Evidence has a date.</strong> Reporting scope and PR references were checked {date(snapshot)}; other entries retain their own evidence dates. It is not a live GitHub feed. Implementation, a passing check, a public demo, and owner acceptance are separate signals. <button className="text-button" onClick={() => navigate('guide')}>How we keep this current →</button></div>
  </>;
}
