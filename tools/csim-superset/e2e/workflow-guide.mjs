...
export const workflowGuide = {
  '10': {id:'dashboard-time-units', issue:'issue-time-menu', label:'Independent Time Unit choices in the snapshot', try:'In the snapshot, open the CSiM Time Unit menu, then try the hourly reporting dashboard and select Day.', expected:'CSiM offers Month/Quarter/Year; the hourly dashboard offers Hour/Day/Week on the same instance. Choices survive reload. Q1 is 10%; each daily specimen total is 72.'},
  ...
  '09': {id:'native-import', issue:'issue-transfer', label:'Native import and remaining production checks', try:'Review the dashboard imported into an independent test instance, then switch to Quarter.', expected:'The import preserves 21 charts, eight datasets with exact SQL and six filter relationships despite changed chart IDs. Q1 remains 10%. Updates to shared production objects are not yet verified.'},
  '08': {id:'filter-options', issue:'issue-time-menu', label:'Check time choices and comparison selectors', try:'Open Time Unit, Your hospital, and Cohort/State.', expected:'Month/Quarter/Year is an instance-wide workaround. The separate upstream snapshot demonstrates dashboard-specific choices. Hospital and comparison selectors and section links work.'},
};

const videoCopy = {
 '10': ['Give each dashboard the reporting units it needs.', 'Independent Time Unit choices work in this pinned upstream snapshot. This feature is not in the stable release.'],
 ...
 '08': ['Review the time, hospital and comparison choices.', 'Month/Quarter/Year currently applies to the whole instance. The separate upstream snapshot demonstrates dashboard-specific choices.'],
