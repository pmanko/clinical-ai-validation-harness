...
import { baseURL, overviewURL } from './settings.mjs';
const dir=process.argv[2], out=path.join(dir,'public');
fs.mkdirSync(out,{recursive:true});
const preview=process.argv.includes('--preview');
const provenance=preview ? {timestamp:new Date().toISOString(),target:'local navigation preview',baseURL,overviewURL,fixture:'No recorded results in this navigation preview'} : JSON.parse(fs.readFileSync(path.join(dir,'provenance.json'),'utf8'));
...
const workflows=specs.filter(spec=>publication.workflowIds.includes(spec.title.slice(0,2))).map(spec=>{
