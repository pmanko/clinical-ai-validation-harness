...
if (!fs.existsSync(resultPath)) { console.error('No test result file; no completed-workflow claim.');process.exit(1); }
...
const outcomes={
  ...
  '07':['Navigation checked',false], '08':['Workaround · per-dashboard choice open',true],
  '09':['Import verified · production update open',true],
};
const workflows=specs.map((spec,index)=>{
  ...
  for(const [i,a] of (run?.attachments||[]).entries()){
    if ((!a.path && !a.body) || !['image/png','video/webm'].includes(a.contentType))continue;
    ...
    const ext=a.contentType==='image/png'?'png':'webm',name=`workflow-${index+1}-${i}.${ext}`;
  ...
  if(film) {
    const file=`workflow-${index+1}.mp4`,captions=`workflow-${index+1}.vtt`;
  ...
  if(film) for(const [i,source] of film.contactSheets.entries()) {
    const file=`workflow-${index+1}-frames-${i+1}.jpg`;
  ...
  return {title:spec.title,...guide,outcome:outcomes[spec.title.slice(0,2)][0],hasOpenGap:outcomes[spec.title.slice(0,2)][1],status:run?.status||'not-run',durationMs:run?.duration||0,captureNotes,assets,videoValidation:film?{framesChecked:film.framesChecked,maxPixelDifference:film.maxPixelDifference,durationSeconds:film.durationSeconds}:null,environment:spec.title.startsWith('09')?'Independent import instance':provenance.target};
});
const summary={...provenance,reporterSha256:crypto.createHash('sha256').update(fs.readFileSync(fileURLToPath(import.meta.url))).digest('hex'),videoTooling:filmManifest?{rendererSha256:filmManifest.rendererSha256,encoder:filmManifest.encoder}:null,completed:workflows.length > 0 && !(report.errors || []).length,workflows};
...
const cards=workflows.map(w=>`<section id="${w.id}"><div class="heading"><h2>${escape(w.label)}</h2><span class="${w.status==='passed'?(w.hasOpenGap?'gap':'pass'):'fail'}">${w.status==='passed'?escape(w.outcome):escape(w.status)}</span></div><div class="instructions"><p><strong>Try this</strong>${escape(w.try)}</p><p><strong>Expected result</strong>${escape(w.expected)}</p></div><p class="workflow-links"><a href="${escape(live)}" target="_blank" rel="noopener">Try in the full dashboard ↗</a><a href="${escape(provenance.overviewURL)}#${w.issue}">Read the related explanation →</a></p>${w.assets.filter(a=>a.type.startsWith('video/')).map(a=>`<video aria-label="${escape(w.label)} recording" controls preload="metadata" poster="${w.assets.find(asset=>asset.type==='image/png')?.file || ''}" src="${a.file}">${w.assets.filter(asset=>asset.type==='text/vtt').map(asset=>`<track kind="captions" srclang="en" label="English" src="${asset.file}">`).join('')}</video>`).join('')}<details><summary>Screenshots (${w.assets.filter(a=>a.type==='image/png').length})</summary><div class="shots">${w.assets.filter(a=>a.type==='image/png').map(a=>`<figure><a href="${a.file}"><img loading="lazy" src="${a.file}" alt="${escape(a.name)}"></a><figcaption>${escape(a.name)}</figcaption></figure>`).join('')}</div></details>${w.videoValidation?`<details><summary>Video frame checks (${w.videoValidation.framesChecked})</summary><p class="note">Title cards, section screens and held results are checked against frames extracted from the encoded video.</p><div class="shots">${w.assets.filter(a=>a.type==='image/jpeg').map(a=>`<a href="${a.file}"><img loading="lazy" src="${a.file}" alt="${escape(w.label)} video frame contact sheet"></a>`).join('')}</div></details>`:''}<p class="note">${escape(w.title)} · ${w.videoValidation ? Math.round(w.videoValidation.durationSeconds)+' second walkthrough' : (w.durationMs/1000).toFixed(1)+' second automated check'} · ${escape(w.environment)}</p></section>`).join('');
const workflowIndex=workflows.map(w=>`<a href="#${w.id}">${escape(w.label)}</a>`).join('');
fs.writeFileSync(path.join(out,'index.html'),`<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>CSiM workflow evidence</title><style>*{box-sizing:border-box}html{scroll-padding-top:20px;scroll-behavior:smooth}body{max-width:1160px;margin:28px auto;padding:0 24px;font:16px/1.6 system-ui;color:#24383c;background:#f5f6f3}h1{font-size:34px;line-height:1.2}h2{font-size:23px;line-height:1.3}header{border-bottom:1px solid #d9e2de;padding:0 0 18px;font-weight:700;display:flex;justify-content:space-between;gap:20px;flex-wrap:wrap}header nav{display:flex;gap:22px;font-weight:500}a{color:#24665f;text-underline-offset:4px}a:focus-visible,summary:focus-visible{outline:3px solid #b87712;outline-offset:4px}.intro{max-width:880px;margin:30px 0}.index{display:grid;grid-template-columns:1fr 1fr;gap:0 24px;margin:22px 0 34px;background:white;padding:14px 24px;border:1px solid #d9e2de;border-radius:8px}.index a{padding:12px 0;border-bottom:1px solid #e4e9e5;font-size:15px}section{border-top:1px solid #ccd5d2;padding:24px 0;margin-top:24px}.heading{display:flex;flex-wrap:wrap;align-items:center;justify-content:space-between;gap:20px}.pass,.fail,.gap{font-size:12px;border-radius:4px;padding:4px 9px;white-space:nowrap}.pass{color:#216b42;background:#e4efe7}.gap{color:#78551f;background:#fbf1de}.fail{color:#9b341e;background:#fbeee6}.instructions{display:grid;grid-template-columns:1fr 1fr;gap:30px;font-size:15px;color:#53686c}.instructions strong{display:block;color:#24383c;margin-bottom:5px}.workflow-links{display:flex;gap:25px;flex-wrap:wrap;font-size:14px;margin:10px 0 22px}video{width:100%;aspect-ratio:36/29;object-fit:contain;max-height:680px;background:#172324;border:1px solid #ccd5d2;border-radius:6px}summary{cursor:pointer;padding:14px 0;color:#24665f;font-weight:600}.shots{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:20px}figure{margin:0}img{width:100%;border:1px solid #ccd5d2}figcaption{font-size:13px}.note{color:#53686c;font-size:13px}.run-details{margin:22px 0;font-size:14px}footer{border-top:1px solid #d9e2de;margin-top:25px;padding:25px 0;font-size:14px}@media(max-width:700px){body{padding:0 18px}.index,.instructions{grid-template-columns:1fr;gap:0}.heading{align-items:start}h1{font-size:29px}h2{font-size:21px}header nav{gap:14px;font-size:14px}.workflow-links{gap:12px}.shots{grid-template-columns:1fr}}@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}}</style><header><span>CSiM / Workflow evidence</span><nav aria-label="Dashboard resources"><a href="${escape(provenance.overviewURL)}">Overview</a><a href="${escape(live)}" target="_blank" rel="noopener">Open full dashboard ↗</a></nav></header><main><div class="intro"><h1>See the supported workflows</h1><p>Each captioned walkthrough follows the real controls, pauses at checked results, and identifies any remaining gap. Choose the question you want to review, then try the same selections in the live synthetic dashboard.</p><p class="note">Recordings need no login. The <a href="${escape(provenance.overviewURL)}#demo-access">overview shows the demo login</a>. Dashboard links open saved defaults; follow the selections shown below.</p><details class="run-details"><summary>About this evidence run</summary><p>${passed} of ${workflows.length} automated workflow checks pass. ${escape(provenance.timestamp)}. Screenshots and videos come from the same automated run. A passing screenshot cannot override an assertion failure.</p><a href="summary.json">Run details and source fingerprints</a></details></div><nav class="index" aria-label="Recorded workflows">${workflowIndex}</nav>${cards}</main><footer><a href="${escape(provenance.overviewURL)}">Back to the dashboard overview</a></footer></html>`);
