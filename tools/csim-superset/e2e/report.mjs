...
const outcomes={
  ...
  '07':['Navigation checked',false], '08':['6.1.0 · instance-wide workaround',true],
  '09':['Import verified · production update open',true], '10':['Fix demonstrated · development snapshot',false],
};
...
const workflows=specs.filter(spec=>publication.workflowIds.includes(spec.title.slice(0,2))).map(spec=>{
  ...
  return {title:spec.title,...guide,outcome:outcomes[spec.title.slice(0,2)][0],hasOpenGap:outcomes[spec.title.slice(0,2)][1],status:run?.status||'not-run',durationMs:run?.duration||0,captureNotes,assets,videoValidation:film?{framesChecked:film.framesChecked,maxPixelDifference:film.maxPixelDifference,durationSeconds:film.durationSeconds}:null,environment:number==='10'?'Upstream development snapshot':spec.title.startsWith('09')?'Independent import instance':provenance.target};
});
