...
def test_default_only_prepares_core_without_accounts_settings_or_reset(
    tmp_path, actions
):
    report = setup.prepare_environment(tmp_path)
    assert actions == [
    ...
    assert report["study_accounts"] == "not_requested"
...
def test_initialize_refuses_any_existing_deployment(tmp_path, actions):
    with pytest.raises(SetupError, match="new deployment"):
        setup.prepare_environment(tmp_path, data_action="initialize")
    assert actions == []
...
def test_explicit_initialize_restores_verified_package_and_materializes_index(
    ...
    report = setup.prepare_environment(
        tmp_path, data_action="initialize", baseline=baseline
    )
    assert actions[2] == [
        "bash",
...
def test_reset_backups_precede_any_application_upgrade(tmp_path, actions, monkeypatch):
    ...
    report = setup.prepare_environment(tmp_path, data_action="reset")
...
def test_study_accounts_are_explicit_and_require_synthetic_data_confirmation(
    tmp_path, actions
):
    with pytest.raises(SetupError, match="synthetic/demo"):
        setup.prepare_environment(tmp_path, study=True)
    assert actions == []
    setup.prepare_environment(tmp_path, study=True, confirm_demo_data=True)
    assert actions[-1] == [
        "python3",
