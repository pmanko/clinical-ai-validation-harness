...
| FP-008 | Consume the reporting instance's real FHIR output through existing Data Pipes/Spark; verify useful query/Dataset/dashboard, record provenance and explain coverage/freshness differences. | Native selected-sample replay in OpenELIS #4323; existing pipeline and Catalyst path used unchanged | Scoped replay emits 15 resources with unchanged clinical rows; Data Pipes refresh, Gemma 12B generation, Qwen review, explicit execution, save/publication/import and server rendering pass | Native #4323 remains open/review-required at `ac64004`; all required checkpoints pass. Catalyst #138 and harness setup #194 are merged | Fresh local recording and server Dashboard `dea5186c…` render selected FHIR result UUIDs at 450 | Pending public video and owner acceptance |
...
| 4 | Scoped native replay, 15 resources, unchanged clinical rows, Data Pipes refresh, both model roles, save/publication/import and [server Dashboard `dea5186c…`](https://catalyst.openelis-global.org/catalyst-dashboards/superset/dashboard/catalyst-dea5186c-bb7b-4b8f-82c8-8ad5abb746f5/) render selected FHIR UUIDs at 450. | Publish the paced cut and obtain owner acceptance. Upstream native review/merge remains separate; selected replay does not prove future automatic emission. |
