...
| FP-002 | Extend the existing integration mock/spec for CSV import, origin-aware Dataset review and PostgreSQL source selection. Preserve approved shell, drafts and explicit execution; review desktop/narrow and light/dark. | Mock and product/contract alignment in Catalyst #127 | Six fixture tests and syntax/link checks pass; browser file/type/failure/cancel/retry, source/query/save, retained drafts and access states inspected; desktop/narrow light/dark and exact logo/title styling checked; all five hosted CI jobs pass | Merged as 8524925 | Exact merged design copied to existing review site; application runtime unchanged | Pending; required before implementing new product behavior |
| FP-003 | Reuse the native reporting effort's actual export and saved-report rerun; verify lane-1 CSV, native Superset upload, table and summary chart. Record early FHIR field coverage against this same OE instance. | Native work exists; integration pending | Four local synthetic result identities and six public CSV downloads verified; separate Superset upload destination missing | Native PR status separate | Public receipt identifies 8005e4c; CSV-to-Superset proof pending | Pending |
