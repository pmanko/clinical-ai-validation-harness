...
| FP-003 | Reuse the native reporting effort's actual export and saved-report rerun; verify lane-1 CSV, native Superset upload, table and summary chart. Record early FHIR field coverage against this same OE instance. | Native work exists; local native Superset upload configured in separate catalyst_imports database | Six public CSV downloads verified; actual native upload preserves text IDs 1158/1159, two 450 values and 30/90-minute validation intervals in a rendered raw table; bar-chart results also match 1158→30 and 1159→90 | Native PR status separate | Published server CSV → local Superset dashboard 31 verified; fresh local export/rerun and server rendering pending | Pending |
