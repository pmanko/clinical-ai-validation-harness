...
| FP-003 | Reuse the native reporting effort's actual export and saved-report rerun; verify lane-1 CSV, native Superset upload, table and summary chart. Record early FHIR field coverage against this same OE instance. | Native preview `ac64004` export/rerun plus Superset native upload | Browser export/rerun and server file upload pass; Dashboard 6 renders IDs 1154/1155, both 450, plus count 2 | Native #4323 remains open/review-required at `ac64004`; all three required checkpoints pass. Repeatable source setup harness #194 merged as `812451c` | Fresh local recording and complete server table/count rendering verified | Pending public video and owner acceptance |
| FP-004 | Ordinary PostgreSQL connection, complete readable catalog, parameters/types/bounds, editor, native question/refinement/Run/save/reopen. Reuse compatible history; retain Spark behavior and no relation allowlist. | Connection/catalog/types/bounds and editor in Catalyst #129; persistent server reader/network/source setup in harness #194 | Local recording proves review/manual correction/save/publication. Server Dashboard `5e39c16f…` renders IDs 1154/1155 at 450 | Catalyst #129 merged as `6513c88`; harness #194 merged as `812451c` | Fresh local recording and actual server Superset rendering pass. Full-catalog preparation exceeded context before model calls; manual correction is the verified completion path | Pending public video and owner acceptance |
| FP-005 | Resolve publication by actual Dataset backing connection and declared dialect. Preserve exact saved artifacts, deterministic bundles, retry/receipts and verify PostgreSQL/Spark rendering. | Query-backed source resolution and dialect-aware compiler merged in Catalyst #130; setup #194 makes backing sources repeatable | Actual PostgreSQL and Spark bundles imported; server browser proofs verify exact selected IDs and values | Catalyst #130 merged as `3b8882f`; harness #194 merged as `812451c` | PostgreSQL Dashboard `5e39c16f…` and Spark Dashboard `dea5186c…` render successfully | Pending owner acceptance |
| FP-006 | Add origin-aware Dataset contracts and CSV upload/review/confirmation with durable import storage. Verify types/order/values, no fabricated SQL, invalid/empty/interrupted import, retry, reload/restart and existing saved artifacts. | CSV upload/type review, durable drafts, immutable PostgreSQL storage and raw-table publication implemented in Catalyst #131; grouping remains FP-007 | Latest 70 focused backend checks including real PostgreSQL, 306 UI checks across full/retry runs, real Gateway import browser journeys in light/dark; desktop/narrow screenshots inspected; all five hosted checks pass at 0011cd7 | Catalyst #131 merged as c91ae23; upload-size repair #132 merged as 4d41978 with all five checks green | Fresh local browser recording and retained server Dataset/Dashboard verify exact native CSV values, immutable reload and SQL-free publication | Pending public video and owner acceptance |
| FP-007 | Shared meaningful chart controls over imported rows, including grouping/counts. Verify values, immutable versions, saved arrangement and publication without required SQL interaction. | Approved controls and aligned mock/contracts implemented in Catalyst #133; #134 repairs PostgreSQL publication for an ungrouped whole-file summary and advances its mapping revision | #133: 440 Gateway, 309 UI, 26 component and five real CSV/browser checks. #134: 443 Gateway checks against PostgreSQL plus all five hosted checks; count/sum/average regression uses 122 persisted rows beyond the preview bound | #133 merged as f548e04; #134 merged as a15ecb8 | Local table/count/arrangement checks pass; retained server Dashboard `0899ee92…` renders IDs 1154/1155 at 450 and grouped count 2 | Chart-control design approved 14 September; public video and final owner acceptance pending |
| FP-008 | Consume the reporting instance's real FHIR output through existing Data Pipes/Spark; verify useful query/Dataset/dashboard, record provenance and explain coverage/freshness differences. | Native selected-sample replay in OpenELIS #4323; existing pipeline and Catalyst path used unchanged | Scoped replay emits 15 resources with unchanged clinical rows; Data Pipes refresh passes. The local recording runs Gemma 12B generation and Qwen review; the separate server proof uses manual SQL with no model request. Both complete save/publication/import and rendering | Native #4323 remains open/review-required at `ac64004`; all required checkpoints pass. Catalyst #138 and harness setup #194 are merged | Fresh local recording and server Dashboard `dea5186c…` render selected FHIR result UUIDs at 450 | Pending public video and owner acceptance |
| FP-009 | Exercise four complete local journeys, compare approved mocks at desktop/narrow and light/dark, inspect rendered values, retain drafts/state, record findings and owner review. | Four fresh local pathway recordings and paced cuts created | Video hashes recorded. Lane 1/3/4 cuts watched at normal speed; lane 2 final concatenated playback remains. Lane 3 records pre-model context rejection plus successful manual correction; lane 4 records both model roles | Required product/setup changes merged; native preview remains separately open | Fresh local paths complete for review; raw evidence remains outside Git | Pending final lane-2 playback and owner acceptance |
| FP-010 | Deploy compatible reviewed revisions with retained data; prove four server journeys and deliver the [four-pathway videos and openclinai.org review checkpoint](../openelis-reporting-catalyst-integration.md#review-checkpoint-four-pathways-videos-and-public-explanation), including readable recordings, live playback, local evidence links and reconciled authoritative documents. | Four server routes and four private local cuts prepared | Browser-rendered proof exists for native CSV, imported CSV, PostgreSQL and FHIR/Spark results | Harness #194 merged/deployed as `812451c`; Catalyst `6ba0008`; native preview `ac64004` | All four server results render at Dashboard 6, `0899ee92…`, `5e39c16f…` and `dea5186c…` | Pending final lane-2 playback, public media/reference verification and owner acceptance |
...
Server deployment and lane-2 proof, 15 September: harness #189 (`f049df6`)
and Catalyst #138 (`364c5bc`) are deployed through the owning wrapper. Strict
repository checks and wrapper health pass. The original five Datasets, six
Widgets and five Dashboards, and every storage mount, were preserved. With
explicit owner confirmation, the separate `catalyst_imports` database and scoped
writer, read-only publication and native-upload accounts were provisioned on the
existing PostgreSQL service. Clinical databases and Superset metadata were retained.

A real browser run against the server reporting instance exported the May5 CSV,
saved and reopened its report definition, verified the fresh-period requirement,
and reopened its queue. The CSV checksum is `c6ace94933a1e2377d28d3d89bf96995e46474b0009c6594c8eece8e1cad56f4`.
A second browser run uploaded those bytes in Catalyst, reviewed text identifiers,
saved/reloaded the Dataset, created a full table and accession-grouped count,
saved/reloaded arrangement and explicitly published. The importer ran from the
server's owning checkout and returned bundle
`4e94f6960d8d45c55fc8fae5b662366d678e5a092b298be4f979b7f2a76d6f69`.
[Server Superset](https://catalyst.openelis-global.org/catalyst-dashboards/superset/dashboard/catalyst-0899ee92-2deb-47b9-a84b-6e518a8f382a/)
renders distinct IDs 1154/1155, both 450, and grouped count 2. The screenshot was
inspected; no SQL/model request occurred. The two recorded tests passed in 1.6
and 2.0 minutes. Raw video, traces and receipts remain outside Git under the
existing private evidence folder's `capture/native-export-server` and
`capture/lane2-server`. This is selected-record proof, not complete fixture,
video/publication or owner acceptance.
...
Current closeout state, checked 16 September:

- Local Catalyst and native reporting are available. Four fresh local cuts and
  hashes are retained privately. Lane 1, 3 and 4 passed normal-speed review; the
  final concatenated lane-2 cut still needs playback review.
- Harness #194 merged as `812451c` and supplies repeatable reader, network and
  source setup. The server runs that harness revision with Catalyst `6ba0008` and
  native OpenELIS preview `ac64004`; retained data was preserved.
- All four server routes have browser-rendered output. Native #4323 remains an
  upstream review/merge concern at `ac64004`; its three required checkpoints pass,
  while code-owner review remains required. Preview deployment was separately
  authorized and does not imply upstream acceptance.
- Server PostgreSQL and Spark verification used manual SQL without model requests;
  wrapper provenance still reports the existing E4B default router profile. The
  Gemma 12B/Qwen evidence below belongs only to the fresh local recordings.
- Public media/reference publication and explicit owner acceptance remain open.
...
| --- | --- | --- |
| 1 | Fresh native export/rerun recording plus server file upload. [Server Dashboard 6](https://catalyst.openelis-global.org/catalyst-dashboards/superset/dashboard/6/) reloads a table with IDs 1154/1155 at 450 and a count of 2. | Publish the paced cut and record owner acceptance; selected-record proof does not claim exhaustive fixture coverage. |
| 2 | Fresh local CSV-import recording and retained [server Dashboard `0899ee92…`](https://catalyst.openelis-global.org/catalyst-dashboards/superset/dashboard/catalyst-0899ee92-2deb-47b9-a84b-6e518a8f382a/) render IDs 1154/1155 at 450. | Watch the final concatenated cut at normal speed, then publish and obtain owner acceptance. |
| 3 | Persistent restricted reader/source, fresh local manual-recovery recording, save/publication/import and [server Dashboard `5e39c16f…`](https://catalyst.openelis-global.org/catalyst-dashboards/superset/dashboard/catalyst-5e39c16f-23ee-4434-bd4d-e78452b338cf/) render IDs 1154/1155 at 450. | Publish the paced cut and obtain owner acceptance. Preserve the context-preparation rejection as an honest limitation; no model tuning is required. |
| 4 | Scoped native replay, 15 resources, unchanged clinical rows and Data Pipes refresh pass. The local recording runs both model roles; separate manual-SQL server verification renders the selected FHIR UUIDs at 450 in [Dashboard `dea5186c…`](https://catalyst.openelis-global.org/catalyst-dashboards/superset/dashboard/catalyst-dea5186c-bb7b-4b8f-82c8-8ad5abb746f5/). | Publish the paced cut and obtain owner acceptance. Upstream native review/merge remains separate; selected replay does not prove future automatic emission. |

The 16 September private evidence index records server revisions, proof paths and
SHA-256 hashes for all four local cuts under
`~/code/catalyst-evidence/four-pathways/2026-09-16/capture/review`. Lane 1 and 2
use no AI. Lane 3 selected the Gemma 12B/Qwen profile, but context preparation
rejected the full native catalog before either role ran; the visible manual SQL
recovery then completed the workflow. Lane 4 ran Gemma 12B generation and Qwen
review before explicit execution and manual refinement. No tuning or performance
test was performed.

The FHIR connection repair reused the existing proxy/network and restarted only
native reporting's app. OpenELIS PR #4323 adds selected-sample replay through the
existing transform; local application revision `fa70a8c` emitted samples 1154 and
1157 (15 resources). The four Observations match native results 1154/1155 on May 5
and 1158/1159 on May 6, all final at 450 copies/ml. Full native result, analysis,
sample and sample-item rows are unchanged before/after replay.

After the existing Data Pipes refresh, the previous manual session correctly
rejected its stale catalog. A fresh session with identical human-authored SQL
returned the four records without model requests. FHIR specimen accessions carry
`-1` (sample-item sort order); CSV accessions are parent orders. Result identity
uses the native-to-FHIR UUID mapping, not accession text equality.

Browser verification saved the Dataset and full-row table, reopened the saved
Dashboard after reload, explicitly published, and ran the owning harness importer.
Superset dashboard `cc95f6ea-4621-467a-9fe4-f0b5f0754fea` renders all four UUIDs,
values, units, statuses and dates; screenshot inspected. Bundle:
`9300cfa4918e5f32a0aa079c3d4fffa2a3e6d95946ab8beeac0c98f665b4a22f`.
Private evidence: `native-fhir-read/spark-record-proof.json`,
`dashboard-publication.json` and `dashboard-import.json` in the existing
four-pathway evidence folder. At that earlier local checkpoint, native PR #4323
was at `cc6a1b8`; the current deployed preview and PR head are `ac64004`, with
server proof recorded above. Automatic future FHIR
emission, broader fixtures and final recordings remain separate checks.
