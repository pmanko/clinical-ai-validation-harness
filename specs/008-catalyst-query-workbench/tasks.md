...
- [ ] T141 [US7] Cover FR-080/SC-041 with failing typed UI/API/E2E tests, then recompose the existing `QueryWorkspace` into the supplied Ask/Datasets/Widgets/Dashboards shell while integrating all accepted behavior: put the one completion/formatting-enabled SQL editor, parameters, Format/Validate/Run actions, diagnostics and result state in the latest chronological turn; keep older turns and the Dataset panel read-only; retain profile/model evidence, manual versions/unresolved snapshots, advisory Validate, explicit Run, findings/raw failure/database diagnostics, typed results, contextual follow-up, timeline, staleness, refresh and New session through `Save Dataset`; introduce no example prompts or automatic execution; move schema/data context and the executed-result preview into the compact thread/Dataset tile/review-panel presentation without losing information or actions; then add five supported visualization families with a compact compatible-type selector/read-only bindings and schematic thumbnails only, libraries, stale states, `Publish to Superset`, download, import status, and `Open Superset` in `targets/catalyst/catalyst-ui/src/features/`
- [ ] T142 [US7] Prove FR-080/SC-041 by first rerunning the accepted query-notebook E2E matrix in the Dashboard Builder shell—profile selection, generation, manual edit/version, Format, Validate, explicit Run, visible diagnostics/results, follow-up successor, rerun, stale labeling, refresh restoration, New session, and `Save Dataset` with exactly one SQL editor—then prove deterministic builder refresh restoration, zero model/database calls during configuration, byte-identical export, clean Superset import, changed publication through a stable Dashboard UUID plus new versioned child UUIDs, and rendered-value parity with PostgreSQL in automated real-path tests; also cover idempotent same-digest import, failed import recovery, keyboard traversal, narrow layout, and actual 200% zoom
