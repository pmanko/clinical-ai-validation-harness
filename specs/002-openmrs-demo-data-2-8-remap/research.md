...
|---|---|---|---|
| **Q1** | When obs → typed row, do we DELETE the obs or KEEP it with `obs.order_id` linkback? | **Typed table is canonical; do NOT keep duplicate obs.** P1/P2/P3 promoted source rows are excluded from residual `obs` by `clin__obs.sql`'s WHERE clause. The only exception is P4: a test-order source obs may remain as a linked result row via `obs.order_id` when it is semantically distinct from the order (i.e. the obs IS the test result, not a copy of the order fact). | No duplicate canonical facts. Indexing the same clinical fact twice in chartsearchai produces hallucination-risk double-returns. OpenMRS typed tables (drug_order, conditions, allergy, test_order) are the authoritative representations; residual obs are distinct un-promoted observations only. |
| **Q2** | UUID strategy on promoted rows? | **Deterministic UUIDv5-style name-based UUIDs** for every synthetic promoted OpenMRS row that requires a UUID. Existing carried-forward rows preserve source UUIDs; existing target dictionary/config rows keep target UUIDs. Synthetic promoted rows use fixed Feature 002 namespace `2f56d7b8-8f8f-5d3a-9f52-002002800001` plus stable source identity and target role, e.g. `feature-002:orders:drug:<source_obs_uuid>` or `feature-002:conditions:<source_obs_uuid>`. | Reproducibility across re-runs (SC-004) without treating UUIDs as clinical mapping evidence. `UUID()` calls in SQLMesh models made replay output drift; deterministic name-based UUIDs keep synthetic object identity stable for REST/FHIR, exports, audits, and future replay checks. |
