...
> **Integration update:** The deterministic data-remap plan remains active. References below to the
> deleted ChartSearchAI `/search`, embedded indexer warmup, or module-owned inference are historical.
> Current cross-target validation uses `make chartsearchai-local`, ChartSearchAI `/chat`, and a
> med-agent-hub product profile over the imported patient corpus.
...
- **Real-path validation against ChartSearchAI** uses `make chartsearch-test` for the module reactor and `make chartsearchai-local` for the assembled product. A live question is sent through `/ws/rest/v1/chartsearchai/chat` with a hub product profile; the proof verifies citations against translated records and persisted session hydration.
...
- **Cross-target ChartSearchAI validation**: `make chartsearch-test` plus the assembled `make chartsearchai-local` relay and Playwright gates.
...
| **M2-F** | Import smoke + RefApp binding check + ChartSearchAI self-tests | US1, FR-014, FR-CD5, SC-002 (binding), SC-013 | `artifacts/<run>/import-smoke/report.json`, `artifacts/<run>/refapp-binding/report.json`, `artifacts/<run>/chartsearchai-tests/results.xml` | Live O3 stack starts cleanly; Liquibase succeeds; `make chartsearch-test` passes; harness assertions cover translated forms, order types, drug catalog, vitals, allergens, and problems. |
| **M2-F.1** | **Live ChartSearchAI profile relay against translated demo** | US1, FR-014, FR-022, **SC-015**, Constitution Principle I | `artifacts/<run>/chartsearchai-live/relay-proof.json`, citation-resolution output, and optional UI evidence | `make chartsearchai-local` starts the assembled stack; a checked hub profile answers through `/chat`; persistence and final citations resolve to translated patient records. |
