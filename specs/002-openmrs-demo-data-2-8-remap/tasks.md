...
- [ ] T020 [P] [US2] Scenario-diversity test in `evals/dataset_import/test_profile_scenarios.py`: assert profile surfaces ambiguous concept reference sources, missing locales, orphan FK candidates, and unbundled-module table cases at record level (FR-024). **Partial coverage in evals/dataset_import/test_profile_inventory.py (12 tests landed alongside T021); remaining scenarios — ambiguous reference sources, missing locales, orphan FKs, unbundled-module cases — depend on T022/T023 outputs and will land alongside those.**
