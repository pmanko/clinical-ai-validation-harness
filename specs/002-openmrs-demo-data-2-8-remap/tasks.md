...
- [ ] T024 [US2] Replace `harness/schema_diff.py` stub with the real implementation: introspect MariaDB `legacy_27_raw` and `refapp_28_clean` schemas; produce items per `contracts/schema_diff.schema.yaml` with `clinical_meaningful` set per the §R5 rule. Depends on T021 (profile inventory feeds reference-table membership).
