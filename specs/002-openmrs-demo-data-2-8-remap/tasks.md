...
- [X] T059 [US1] **Implementation via SQLMesh+dlt handover (Phase 5D).** Replaces the original "load `refapp_28_demo.sql`" approach which was incompatible with SQLMesh's virtual-layer storage. Sub-tasks below.
  - [X] T059a [US1] **Hermetic test surface** (commit `dbf0182`): `scripts/loadtest-up.sh` + `loadtest-down.sh` manage both `openmrs_test` and `openmrs_test_dlt` schemas; clone from CIEL-loaded `openmrs`. Makefile `loadtest-up`/`loadtest-down` targets.
  - [X] T059b [US1] **FK reconciliation seed maps** (commit `11503d5`): six identity maps under `datasets/transforms/sqlmesh/models/terminology/`. Documentation-only per Path B decision (see `datasets/load/openmrs-loadback.review.md`); activate Path A on first specific FK collision during iteration.
  - [X] T059c [US1] **dlt loader package** (commits `d9e9b92` + `e7508af`): `harness/load/{__init__,snapshot_resolver,pipeline,promote,__main__}.py` + `dlt[sqlalchemy]>=1.0` in pyproject.toml + two-schema architecture (dlt → `<target>_dlt`, promote → `<target>`) per `contracts/dlt_pipeline.profile.md`. Includes `clin__orders` parent model and `obs.obs_id` order_id strategy. Verified: 33 resources load → 5,284 patients / 428,013 obs / 43,412 drug_orders / 1,095 test_orders / 4,451 conditions / 2 allergies in `openmrs_test`. RunManifest002Extensions extended with `dlt_pipeline_run_id`, `dlt_state_hash`, `dlt_version`.
  - [X] T059d [US1] **Real `harness/import_smoke.py`** (this commit chain): replaces stub with REST + FHIR readback + DB count cross-checks. Samples N patients deterministically, fetches via `/ws/rest/v1/patient/<uuid>` and `/ws/fhir2/R4/Patient/<uuid>`, counts their encounters/obs. Emits `artifacts/<run>/import-smoke/report.json` per FR-016 record-level evidence. Verified: 5/5 sampled patients resolve with 3-4 encounters and 95-120 obs each; FHIR Patient total = 5,284.
- [X] T057 [US1] **Orphan FK detection** (this commit chain): `harness/transform/orphan_fk.py` enumerates `information_schema.referential_constraints` for `openmrs_test`, surfaces orphans per FK with sample offenders. Emit `artifacts/<run>/transform/orphan-fk-report.json` per FR-013. Verified: 847 FKs checked; current state has 1,967 orphans concentrated in stock-data residue (encounter_diagnosis, obs_reference_range, visit, patient_appointment) — fix path documented in `datasets/load/openmrs-loadback.review.md`.
...
- [ ] T064b [US1] **Obsolete after the 2026-07 hub-relay refactor:** this task referenced the old chartsearchai warmup/indexing surface. Current live validation should exercise the hub-backed chat path and querystore-owned indexing instead of invoking the removed module warmup endpoint.
- [ ] T064c [US1] Live relay-flow test: send a hub product profile through `/ws/rest/v1/chartsearchai/chat` for at least one known translated patient; assert the Answer/validation/In-Depth/evidence envelope and persisted hydration; save the proof under `artifacts/<run>/chartsearchai-live/`.
