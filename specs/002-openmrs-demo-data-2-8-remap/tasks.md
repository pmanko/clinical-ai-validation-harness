...
- [ ] T059 [US1] **Reframed — see T059a-d sub-tasks.** Implement the load step via dlt + replace `harness/import_smoke.py` stub with real REST+FHIR readback. Per `contracts/dlt_pipeline.profile.md` + research.md §R-load-pattern. The original "load `refapp_28_demo.sql`" approach was superseded: `mariadb-dump refapp_28_demo` produces only view definitions (no data) because SQLMesh stores everything as views over `sqlmesh__refapp_28_demo` snapshots. dlt reads those physical snapshots directly and writes to openmrs_test with PK-based idempotency. Closure of T059 = closure of T059a + T059b + T059c + T059d.
  - [ ] T059a [US1] **Hermetic test surface**: `scripts/loadtest-up.sh` (drop + recreate `openmrs_test` schema, apply CIEL seeded-baseline) + `scripts/loadtest-down.sh` + Makefile targets. Same MariaDB container, separate schema.
  - [ ] T059b [US1] **FK reconciliation seed maps**: `datasets/transforms/sqlmesh/models/terminology/{user_map,location_map,encounter_type_map,encounter_role_map,role_map,provider_map,visit_type_map}.sql`. Default identity maps; document why in companion review doc.
  - [ ] T059c [US1] **dlt loader package**: `harness/load/{__init__,snapshot_resolver,pipeline,__main__}.py` + `dlt[sqlalchemy]>=1.0` in pyproject.toml + extend `RunManifest002Extensions` with `dlt_pipeline_run_id` + `dlt_state_hash` + `dlt_version` fields + wire `harness-cli load-test`. Per `contracts/dlt_pipeline.profile.md`.
  - [ ] T059d [US1] **Real `harness/import_smoke.py`**: replace stub with REST + FHIR readback against `openmrs_test` (or `openmrs` post-promotion). Surface per-patient evidence (not aggregate); emit `artifacts/<run>/import-smoke/report.json`; stamp into manifest's `materialized_outputs[]`.
