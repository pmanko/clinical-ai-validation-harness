...
**Independent Test**: Can be fully tested by asking each of the five canonical questions against a patient with lab data present in OE2 (via the HAPI FHIR sidecar) and confirming each answer includes citation markers whose referenced FHIR resource IDs (`Patient`, `Observation`, `ServiceRequest`, `DiagnosticReport`) resolve to real resources in OE2.

**Acceptance Scenarios**:

1. **Given** a patient in OE2 with recorded lab observations, **When** a reviewer asks "Show recent lab results for patient X," **Then** the answer lists the results and cites the specific `Observation` resource IDs used, and each ID resolves against OE2's HAPI FHIR endpoint.
...
After the HAPI-grounded answer path works, the same five canonical questions are replayed against OE2's embedded FHIR providers (the production-representative surface), and any divergence between the two surfaces is recorded as a documented gap rather than silently ignored or treated as a blocking failure.

**Why this priority**: This is what makes the POC's evidence credible beyond "it works against the permissive dev-only HAPI container" — it's a diagnostic pass, not a launch blocker, so it is correctly the lowest priority: valuable, but the POC still delivers value without it if time runs short.

**Independent Test**: Can be fully tested by replaying the five canonical questions' underlying FHIR reads against OE2's embedded FHIR providers and confirming a gap-log entry is produced for every resource read that differs (or is unavailable) between HAPI and embedded, without that divergence blocking the HAPI-path answer already produced in Story 1.
...
2. **Given** a resource is available on HAPI but unavailable or materially different on the embedded surface, **When** the parity probe runs, **Then** a gap-log entry records the resource, the surface(s) involved, and the nature of the divergence, and the original HAPI-grounded answer remains valid and visible.
...
- The OE2 HAPI FHIR endpoint is unreachable or times out mid-query.
...
- The embedded FHIR surface requires authentication the parity probe does not have — the probe records this as a gap rather than crashing the run.
...
- **FR-001**: The system MUST answer all five canonical questions (§6 of the source brief) using live reads against OE2's HAPI FHIR sidecar as the primary data path.
...
- **FR-010**: After the HAPI-grounded answer path is validated, the system MUST support replaying the five canonical questions' underlying FHIR reads against OE2's embedded FHIR providers and recording any divergence (missing resource, differing content, unavailable surface) as a gap-log entry rather than failing the run.
- **FR-011**: A recorded parity-probe divergence MUST NOT invalidate or hide the original HAPI-grounded answer produced for the same question.
...
- **SC-005**: The embedded-FHIR parity probe completes a full pass over all five canonical questions' underlying resource reads and produces a gap-log entry for every detected divergence, with zero of those divergences causing the original HAPI-grounded answer to be withdrawn or hidden.
...
- No additional authentication is added to OE2's HAPI FHIR sidecar for this POC; the dev container's existing lack of application-layer auth is accepted as a POC-only, non-PHI, localhost/dev-network condition. Hardening HAPI auth is out of scope and tracked as an OE2-side follow-up.
