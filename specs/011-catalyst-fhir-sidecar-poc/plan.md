...
- **Metadata and provenance**: PASS. `RunManifest.component="catalyst"` plus each result row's persisted `provenance` object (fhir_surface, fhir_base_url, tools_called, resource_ids) gives the same run-manifest/event-trace chain other targets already have; no new manifest schema needed (see data-model.md, Harness Adapter Entrypoint section).
