...
const targetNodes: ArchNode[] = [
  { title: 'one cell', detail: 'scenario × arm', x: 20, y: 130 },
  { title: 'ANSWER call · fast', detail: 'single → router · team → hub', x: 280, y: 40, tone: 'success' },
  { title: 'Answer · latency_A', detail: '→ Benchmark · returned immediately', x: 560, y: 40, tone: 'success' },
  { title: 'IN-DEPTH call · slow', detail: 'per-arm writer · rooted in the answer', x: 850, y: 224, tone: 'info' },
  { title: 'In-Depth · latency_I', detail: '→ Background · follows asynchronously', x: 1130, y: 224, tone: 'info' },
];
const targetEdges: Array<[number, number]> = [[0, 1], [1, 2], [2, 3], [3, 4]];
...
const phaseNodes: ArchNode[] = [
  ...
  { title: 'P3 · judge + report', detail: '2 axes · ALL arms — done', x: 800, y: 30, tone: 'success' },
  { title: 'P4 · async delivery', detail: 'answer first · separate latency — done', x: 1060, y: 30, tone: 'success' },
];
...
const roadmapRows: ReactNode[][] = [
  ...
  ['P3', 'Judge + report consume two independent axes for ALL arms — judge-prep reads the nested in-depth; both quality axes published', done],
  ['P4', 'Async delivery: answer returned first, in-depth follows, each separately timed (ans ~2-33s, in-depth ~12-24s)', done],
];
...
export default function AnswerInDepthParityCanvas() {
  return (
    <Stack gap={22}>
      ...
      {/* ---------- VISION ---------- */}
      <Stack gap={10}>
        <H2>The target — answer first, in-depth follows</H2>
        <Text tone="secondary">
          Each cell makes two calls. The <b>Answer</b> returns first (fast); the <b>In-Depth</b> is <b>rooted in that
          answer</b> — produced by the arm&apos;s own writer (per-arm) — and follows <b>asynchronously</b>, so you get a
          quick answer and a slower in-depth, each separately timed and judged. Vanilla singles finally get an In-Depth
          too; team scaffolding shapes the Answer.
        </Text>
        <Card>
          <CardBody><ArchDiagram id="target" nodes={targetNodes} edges={targetEdges} width={1370} height={320} /></CardBody>
        </Card>
      ...
      <Stack gap={10}>
        ...
        <Callout tone="warning" title="The gap (current → vision)">
          ...
          3) In-Depth is <b>welded to the answer in one call</b> — not a separate artifact with its own timing. &nbsp;
          4) The fast answer can&apos;t return early — it waits for the in-depth.
        </Callout>
      </Stack>
      ...
      <Stack gap={10}>
        ...
        <Grid columns={2} gap={12}>
          <Callout tone="success" title="Decision 1 — coupling · RESOLVED">
            In-Depth is <b>rooted in the answer</b> (it elaborates the answer), but the <b>answer returns first</b> and
            the In-Depth follows asynchronously — a quick answer, a slower in-depth. Coupled in content; decoupled in
            delivery + latency.
          </Callout>
          <Callout tone="success" title="Decision 2 — model · RESOLVED">
            <b>Per-arm</b> — each arm&apos;s own writer produces its In-Depth, so the in-depth axis is a class survey
            mirroring the answer survey (Gemma-12B vs Qwen-14B vs MedGemma … on elaboration).
          </Callout>
        </Grid>
