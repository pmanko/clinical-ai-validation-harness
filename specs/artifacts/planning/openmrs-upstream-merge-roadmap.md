...
| 2026-09-14 | I3 | none (upstream PR state only) | done | nine PRs closed with content-based supersession comments, branches kept; audit: no head is an ancestor of either integration branch, none of their key classes exist there, successors do; two deltas recorded as not carried by decision (#20 prompt prefix, #22 runtime bundled-model swap); AC 1.6: open PRs by us on both upstreams are #157 and #23 only |
| 2026-09-15 | I2 (second maintainer round) | querystore `d10e9e41` -> `cfced363` (fast-forwarded to `harness-integration`) | done | five new threads answered same day per rule 3: ES WARN cause `69e12fe` (red on the extracted original text), stopwords back/down/i `87619c4` (red: `chronic pain management plan`), ContextSlice ctors `936bee7`, Cache-Control + property-pinned ceiling test `cfced36` (red: 400 on the static read, null header); reactor API 525 + OMOD 53, 0 failures; CI Java 8/11/17/21 SUCCESS; AC 1.1 = 0 of 21 |
| 2026-09-15 | hub sync + catalogue | chartsearchai `d46f517b` -> `fed441e` (paired QueryStore pin `f2fca727` -> `cfced36`, workflow only) | done | catalogue page published at `https://openclinai.org/docs/openmrs-upstream/` (harness #184; an interim claude.ai artifact was retired the same day); #68, #157, #23 bodies aligned to it; the `.guards.json` paired-pin rule is satisfied again; no CI on `fed441e` because pull_request workflows skip a CONFLICTING PR, verified by grep that the branch calls none of the API surface #68 removed |
