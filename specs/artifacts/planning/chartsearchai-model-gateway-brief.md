...
- **FR-008.6**: Provider-specific request quirks MUST be transparent to chartsearchai. In particular: when the chosen model id starts with `claude-opus-4-7`, the request reaching the Anthropic provider MUST omit `temperature`/`top_p` and MUST honor `top_k=1` (matches `RemoteLlmEngine.buildRequestBody` lines 175-180). Equivalent quirks for other cloud APIs (e.g. Azure deployment naming, Gemini `safety_settings`) MUST be handled at the gateway layer — whether by code we own or by an adopted OSS gateway (see Open Q9) — never by chartsearchai. The pre-spec spike (Section 10) confirms which implementation strategy honors these quirks before this FR is finalized.
- **FR-008.7**: The chartsearchai-facing wire MUST emit OpenAI-shape SSE chunks (`data: {"choices":[{"delta":{"content":"..."}}]}\n\n`) regardless of upstream provider, terminating with `data: [DONE]\n\n`. Upstream provider streaming shapes (Anthropic's `event: content_block_delta`, Ollama's NDJSON, etc.) MUST be normalized before chartsearchai sees them. Whether the normalization is implemented in our gateway code or inherited from an adopted OSS gateway is a pre-spec decision (Open Q9 + Section 10 spike); this FR specifies the contract chartsearchai consumes, not the implementation that produces it.
...
- **FR-008.10**: The gateway MUST be integrated as a git submodule at `targets/chartsearchai-model-gateway/`. If it is a fork of an upstream repo, pin its `harness-integration` branch following the same pattern as `targets/chartsearchai/` and `targets/chartsearchai-esm/`. If it is our own repo (no upstream), pin `main` and gate it with its own CI — the pattern `targets/med-agent-hub/` now follows (its harness-integration buffer was retired 2026-06-11).
...
**Recommendation surfaced for `/speckit-clarify`**: Option C deserves a research spike **before `/speckit-specify` runs**, not between specify and plan. Rationale: the build-vs-adopt outcome reshapes the spec's FRs (FR-008.5/.6/.7 in particular), not just the implementation plan. If LiteLLM/Portkey/OpenLLM preserves chartsearchai's wire-shape quirks (json_schema response_format, top_k=1 for claude-opus, SSE translation across providers) verbatim, the spec ships with "verify-the-adopted-gateway-honors-X" acceptance tests. If not, the spec ships with "implement-X-in-our-gateway" build tasks. Either is fine; the spec needs to know which before authoring.
...
| # | Risk | Mitigation |
|---|---|---|
| **R1** | Existing OSS LLM gateways (LiteLLM, Portkey, OpenLLM) already solve this problem; building from scratch duplicates work. | **Pre-spec research item** — see Section 10. Spike LiteLLM compatibility against chartsearchai's wire quirks (json_schema response_format, top_k=1 for claude-opus, SSE translation, streaming with `stream_options.include_usage`) **before `/speckit-specify`**, so the spec's FRs reflect the chosen implementation strategy (verify-OSS-honors-X vs build-X) rather than getting rewritten at plan time. |
...
1. **OSS LLM gateway compatibility spike** (load-bearing for Q9 / R1). **Run before `/speckit-specify`** — outcome reshapes FR-008.5/.6/.7 from "the gateway MUST do X" to "verify our chosen implementation honors X." Spend 1-2 days standing up LiteLLM (or Portkey or OpenLLM) in front of LM Studio + Anthropic + med-agent-hub. Drive it from a curl harness that mimics `RemoteLlmEngine.buildRequestBody` exactly (json_schema response_format, top_k=1 for claude-opus, stream_options.include_usage). Pass criterion: the OSS gateway returns chartsearchai-parseable JSON for every provider class and emits chartsearchai-parseable SSE for every streaming provider. Failure criterion: any wire-shape divergence requires gateway-side patching. Outcome determines build-vs-adopt before the spec is authored.
