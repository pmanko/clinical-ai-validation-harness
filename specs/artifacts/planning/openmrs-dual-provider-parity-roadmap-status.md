...
| G06 Bundled preservation | In progress | Rebuild starts from upstream `58c0daf`; the upstream 635-test baseline passed before changes and the suite now runs **686 tests, 0 failures, 0 errors, 34 skipped** (`mvn test`, 2026-07-22). Legacy bundled wire (`POST /search`, `/search/stream`) is retained. **Proven live 2026-07-22:** the bundled provider streams `reasoning_delta`/`answer_delta` → `answer_done` → `turn_done` on the assembled stack. **Runtime sweep 2026-07-23 (remote engine):** queryScoped ± grounding both answered live with replay-verified engine requests; the legacy bundled wire (`POST /search`) returned a 4-reference answer; hub-only isolation separately proven (G04). **Defect found: fullChart mode fails `unsupported_mode`** on a live turn after flipping `chartsearchai.chartMode=fullChart` (with settle time), and conversations bind `provider_mode=query_scoped` at `/chat/new` regardless of the GP — a real post-rebuild mode-lifecycle regression, confirmed DETERMINISTIC (the registry advertised `full_chart_stable` at the same instant the turn failed — GP-cache timing ruled out by live experiment); needs a red-first unit repro through the real `streamProviderTurn` path in chartsearchai. **Remaining:** that fix, the local-engine (GGUF) leg, warmup/caching legs. |
| G07 QueryStore semantics | In progress | QueryStore `856bdda` implements per-resource `getClinicalDate`/`getDateKind` (`clinical_event`/`administrative`/`unknown`) + `lastModified`, with serializer unit tests; the full-chart read API (`fd8a00c`) shares serializer/service behavior with ranked reads. **Live 2026-07-22:** the hub consumes the querystore chart end to end (grounding cites querystore records; a dateKind-rendering regression was found and fixed against the live path — see progress note). **Two defects found by the 2026-07-23 e2e freshness run (now G07 blockers):** (1) **voided→delete not firing** — a REST-voided obs stayed in the read store (>30 s; Decision 10 violated live; creates DO project, so events flow for saves); (2) **per-patient `POST /reindex` destroyed the chart** — `bulkDeleteByPatient` ran but reprojection indexed 1 document (the voided obs, compounding defect 1) and `indexingstatus` still reported `complete:true`. Recovered via the ES snapshot restore (514,682 docs). Root-cause TBD — note the sync design targets platform 2.9 #6084 events while this stack runs 2.8. **Remaining:** red-first fixes for both defects in querystore, then full-vs-ranked parity and date-correctness fixtures through the harness. |
...
| G11 Context ceiling | In progress | Bundled fullChart fails loud (`ChartTooLargeException`); the shared slice surfaces backend-cap truncation explicitly (`chartTruncated`); the hub trims its token budget over slice tiers with `mandatory` never droppable and slice-selected never zero-relevance (CP3, med-agent-hub `d7b5e44`; suite 611/0 incl. mandatory-overflow behavior). **Remaining:** bundled scoped slices still have no token ceiling of their own (small by construction; explicit-overflow parity for bundled is the residual gap). |
