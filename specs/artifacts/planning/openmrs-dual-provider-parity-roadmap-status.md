...
| Current boundary | Current OpenMRS upstream is merged into the tested integration line. QueryStore `f37adc8`, ChartSearchAI `639c637`, and ESM `971533a` are the exact remote `harness-integration` heads and the heads of OpenMRS PRs #68, #157, and #23 respectively. QueryStore and ESM are non-draft, mergeable, clean, and green in GitHub; ChartSearchAI is non-draft, mergeable, and clean after merging late upstream `dd651ef`, and the committed umbrella source-pair build passes. ChartSearchAI's standalone GitHub Java jobs remain red only because the public QueryStore snapshot predates #68 and therefore lacks the new context-slice classes; #68 must merge/publish before those jobs can compile without a temporary cross-repository CI override. Review findings have regression coverage on all three heads. The parent pin and strengthened publication-readiness gate are carried in harness PR #45. Live product evidence still requires a rebuild from these exact pins; older recordings remain historical evidence only. |
...
| G21 QA and hygiene | In progress | The source-tested heads are QueryStore `f37adc8`, ChartSearchAI `639c637`, ESM `971533a`, and hub `092b5cd`. The stable QueryStore, ChartSearchAI, and ESM suites pass locally, including the committed umbrella QueryStore-to-ChartSearchAI source-pair build; QueryStore and ESM public CI pass. ChartSearchAI's standalone Java matrix is red because it resolves the older public QueryStore snapshot, which does not contain #68's `ContextSlice`, `ContextSliceRecord`, or `ContextSliceRequest`; no ChartSearchAI test executes before that compile failure. The OpenMRS heads equal their remote `harness-integration` heads; the hub is pinned from `main`. The root relay/source-pair tests verify parent pins, clean submodule trees, dependency order, and explicit session identity. The repository-line and deployed-source checks now make branch ownership and runtime provenance executable, and the publication check additionally rejects draft, conflicting, unchecked, pending, or failing OpenMRS PR heads. Remaining before this source line replaces the prior live-proven set: merge/publish QueryStore #68, rerun ChartSearchAI #157 CI, merge harness PR #45, redeploy, complete the live product/browser proof, and perform independent QA. |
