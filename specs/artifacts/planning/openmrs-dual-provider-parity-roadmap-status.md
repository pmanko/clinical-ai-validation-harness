...
| G21 QA and hygiene | In progress | The source-tested heads are QueryStore `f37adc8`, ChartSearchAI `639c637`, ESM `971533a`, and hub `092b5cd`. The stable QueryStore, ChartSearchAI, and ESM suites pass locally; QueryStore and ESM public CI pass, while ChartSearchAI CI is pending after its late-upstream merge. The OpenMRS heads equal their remote `harness-integration` heads; the hub is pinned from `main`. The root relay/source-pair tests verify parent pins, clean submodule trees, dependency order, and explicit session identity. The repository-line and deployed-source checks now make branch ownership and runtime provenance executable, and the publication check additionally rejects draft, conflicting, unchecked, pending, or failing OpenMRS PR heads. Remaining before this source line replaces the prior live-proven set: finish public ChartSearchAI CI, merge harness PR #45, redeploy, complete the live product/browser proof, and perform independent QA. |
...
stable full ChartSearchAI wrapper passes at the merged head. GitHub now reports PR #157 mergeable
and clean; public CI must still complete before the PR is called ready.
