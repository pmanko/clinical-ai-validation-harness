...
| G21 QA and hygiene | In progress | The source-tested heads are QueryStore `f37adc8`, ChartSearchAI `17a91a9`, ESM `f26868c`, and hub `092b5cd`. The stable QueryStore suite, full ChartSearchAI wrapper, and ESM 220-test/lint/TypeScript/production-build suite pass locally. QueryStore #68 remains green; ChartSearchAI #157 and ESM #23 are non-draft and have refreshed CI pending at the new heads. ChartSearchAI's paired Java 11/17/21 matrix rebuilds exact QueryStore source `f37adc8`, while normal OpenMRS branches keep the official standalone workflow. **Remaining:** wait for the exact-head CI, update and merge harness PR #45, redeploy the exact pins, run the live product/browser/demo gates, then produce the hash-bound independent QA and evidence bundle. |
...
locally. Exact-head CI and live browser proof are still required before this follow-up is called
complete.
