...
| G05 Provider selection | In progress | ChartSearchAI commit `5129dc8` adds configuration-driven discovery/default/readiness (`chartsearchai.providers.enabled`/`.default`) with bundled as the fresh-install default and no implicit fallback. REST metadata is wired: `GET /rest/v1/chartsearchai/providers` returns descriptors with `pickerVisible`/`defaultProvider` (`e2bd0db`), and `ConversationService.startNew` closes the active conversation and opens a new one on provider/mode switch. The ESM provider picker (`e602faf`) appears only when `pickerVisible`, starts a fresh conversation on switch, disables unavailable providers, and never falls back — covered by component and hook tests. Assembled-runtime proof remains for Signoff 2. |
