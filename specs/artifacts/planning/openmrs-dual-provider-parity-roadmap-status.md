...
| Current boundary | Signoff 1 granted; context-surface / authoritative-state amendment approved 2026-07-21; shared context-selection (QueryStore context slice) amendment approved 2026-07-22 with checkpoint plan `querystore-context-slice-plan.md`; ChartSearchAI rebuild and remaining roadmap runtime work authorized to proceed. ESM PR #22 was refreshed at `a1da7cb` and its public build is green; ChartSearchAI PR #90 was fast-forwarded to the source-pair-tested `4006c24`, but its upstream build awaits publication of QueryStore #63's new API artifact. Signoff 2 remains required before release/evaluation work. |
...
was fast-forwarded without force-push to `4006c24`; and ESM PR #22 was updated to `a1da7cb`, whose
public OpenMRS CI build passed (run `30501911580`). The root harness points to the same ESM head
once its pin-update commit is made.
