...
| Current boundary | Current OpenMRS upstream is merged into the tested integration line. QueryStore `f37adc8`, ChartSearchAI `17a91a9`, and ESM `f26868c` are the exact remote `harness-integration` heads and the heads of OpenMRS PRs #68, #157, and #23 respectively; med-agent-hub `092b5cd` is pinned from its `main`. All three OpenMRS PRs are non-draft, mergeable, clean, and green at those exact heads. ChartSearchAI's integration-PR CI checks out and installs exact public QueryStore commit `f37adc8` before running its full Java 11/17/21 matrix; normal OpenMRS branches retain the official reusable standalone workflow. Review findings have regression coverage on all three heads. The parent pin and strengthened gates are carried in harness PR #45. The exact pins have also been rebuilt and exercised through the live relay, browser lifecycle, multi-turn, low-confidence disclosure, table hydration, drug-safety warning, and preemption checks. A current-head raw and 2x demo recording is available; the final hash-bound independent QA bundle and parent merge remain outstanding. |
...
- **Not yet proven live:** the assembled local application has not been rebuilt from the four
  exact revisions above. Existing browser/video evidence belongs to the preceding revision set.
...
| G16 Drug-safety honesty | Passed 2026-08-05 | The checked/limited/unavailable result contract is implemented across hub, bundled provider, persisted envelope, and ESM. Hub `SafetyCheckResult` and bundled `DrugSafetyValidator.validateWithStatus()` distinguish a clean completed check from a limited or unavailable check; both consume the shared fixture family. The ESM carries `safetyStatus` through live events and hydration, stays silent for checked-clean, and visibly labels limited/unavailable even when the warning list is empty. Current ESM head `f26868c` includes the original status plumbing, reviewer remediation in `143cbf3`, and the visible answer-check summary fix: safety chips no longer repeat backend enum text, review disclosure is localized, and validation summaries are no longer hidden in native badge tooltips. **Current-head proof:** the normal live relay returned and rehydrated `safety_status=checked` with zero warnings; a second source-specific product probe proposed paracetamol 6000 mg/day, required a verified `drug-safety` citation, returned one deterministic overdose warning (`6000 mg/day` exceeds `4000 mg/day`), preserved the needs-review answer disclosure, and rehydrated the exact same final-envelope hash. The generic probe still requires QueryStore by default; `--expected-reference-source drug-safety` makes the safety proof rigorous without pretending every valid knowledge-base answer must also cite a chart record. |
| G17 Canonical UI | Passed 2026-08-05 | One reducer handles the bundled and hub lifecycle, including Answer checking, optional In-Depth, evidence, original/rejected output, safety state, cancellation, and reload hydration. Checked/edited Answers enter subsequent-turn history before the optional In-Depth tail; `needs_review` output does not. Current ESM `f26868c` also keeps bundled chat independent of hub-profile discovery (`ea1bcef`), records the backend-advertised provider default in shared state without starting a conversation, passes the selected provider when a new conversation is explicitly created, translates session-expiry handling, clears provider state on logout/user change, waits for asynchronous default selection in its regression test, and renders answer-check summaries as visible, status-colored content instead of title-only text. Its 220-test, lint, TypeScript, production-build, and public CI checks pass. ChartSearchAI `17a91a9` supplies the human-readable provider label `Med-Agent Hub` from authoritative backend metadata. **Current-head proof:** a real two-turn E4B browser run showed the visible answer-check summary on both completed turns; the low-confidence fixture preserved rejected/original output across reload; the deterministic provider-envelope fixture rendered and rehydrated a structured medication table. E4B did not emit a table for one live sampled medications answer; that is recorded as model prompt-compliance evaluation work, not misreported as a renderer failure. |
...
| G19 Honest demo | Passed 2026-08-05 | `make chartsearchai-local` builds/stages the current ESM, verifies the live QueryStore source, configures the providers, and runs the hardened relay probe against a newly created explicit hub session. **Current exact proof:** QueryStore `f37adc8`, ChartSearchAI `17a91a9`, ESM `f26868c`, and hub `092b5cd`; all 365 patient records were fetched over four server-capped pages; `answer_done` arrived in 9.125 s and terminal state in 17.653 s on the original warm sample; the persisted final envelope rehydrated with an identical hash, QueryStore references, an edited checked Answer, and honestly withheld `needs_review` In-Depth. The current-head live two-turn and preemption Playwright specs pass. A new paced 1280x720 three-turn/preemption recording passed in 4.5 minutes and was frame-reviewed for the expanded panel, human provider/profile labels, visible answer-check summary, evidence, and honest In-Depth interruption: `artifacts/reports/demos/videos/chartsearchai-dual-provider-e4b-multiturn-preempt-20260805.mp4`; its 2x copy is 2 minutes 16 seconds at the same path with suffix `-2x.mp4`. Model errors remain visible; rejected In-Depth is not presented as complete. |
...
| G21 QA and hygiene | In progress | The source-tested heads are QueryStore `f37adc8`, ChartSearchAI `17a91a9`, ESM `f26868c`, and hub `092b5cd`. The stable QueryStore suite, full ChartSearchAI wrapper, and ESM 220-test/lint/TypeScript/production-build suite pass locally. QueryStore #68, ChartSearchAI #157, and ESM #23 are non-draft, mergeable, clean, and green at those exact heads; ChartSearchAI's paired Java 11/17/21 matrix rebuilds exact QueryStore source `f37adc8`, while normal OpenMRS branches keep the official standalone workflow. The exact deployment passed the relay, multi-turn, low-confidence disclosure, deterministic table hydration, drug-safety warning, preemption, and current-head demo checks. `verify-doc-drift.sh`, the foundation gate, repository-line gate, and network-backed publication gate pass. **Remaining:** produce the hash-bound independent QA evidence bundle and merge harness PR #45 after its final CI/review. |
...
At the exact publication heads QueryStore `f37adc8`, ChartSearchAI `c8ec052`, and ESM `971533a`,
...
`make repository-publication-check` commands pass. PR descriptions now present the same review
order and make clear that the paired source checkout is CI verification, not a merge-time or
...
`answerValidation.summary` only through a native `title` tooltip; `f26868c` now renders that text
...
drug-safety relay proof now pass. The final independent QA evidence bundle remains before release
closeout.
