...
| Current boundary | Current OpenMRS upstream is merged into the tested integration line. QueryStore `f37adc8`, ChartSearchAI `c8ec052`, and ESM `971533a` are the exact remote `harness-integration` heads and the heads of OpenMRS PRs #68, #157, and #23 respectively. QueryStore and ESM are non-draft, mergeable, clean, and green in GitHub; ChartSearchAI is non-draft, mergeable, and clean after merging late upstream `dd651ef`, and the committed umbrella source-pair build passes. Because the public QueryStore snapshot predates #68, ChartSearchAI's integration-PR CI now checks out and installs exact public QueryStore commit `f37adc8` before running its full Java 11/17/21 matrix; normal OpenMRS branches retain the official reusable standalone workflow. Review findings have regression coverage on all three heads. The parent pin and strengthened publication-readiness gate are carried in harness PR #45. Live product evidence still requires a rebuild from these exact pins; older recordings remain historical evidence only. |
...
| G21 QA and hygiene | In progress | The source-tested heads are QueryStore `f37adc8`, ChartSearchAI `c8ec052`, ESM `971533a`, and hub `092b5cd`. The stable QueryStore, ChartSearchAI, and ESM suites pass locally, including the committed umbrella QueryStore-to-ChartSearchAI source-pair build; QueryStore and ESM public CI pass. ChartSearchAI `c8ec052` replaces the impossible integration-PR standalone compile with a paired matrix that rebuilds exact public QueryStore source `f37adc8` before the full ChartSearchAI Java 11/17/21 build; its workflow contract is parsed and pin-checked by the harness test suite. Normal OpenMRS branches still use the official reusable build. The OpenMRS heads equal their remote `harness-integration` heads; the hub is pinned from `main`. The root relay/source-pair tests verify parent pins, clean submodule trees, dependency order, and explicit session identity. The repository-line and deployed-source checks now make branch ownership and runtime provenance executable, and the publication check additionally rejects draft, conflicting, unchecked, pending, or failing OpenMRS PR heads. Remaining before this source line replaces the prior live-proven set: complete ChartSearchAI #157's paired CI, merge harness PR #45, redeploy, complete the live product/browser proof, and perform independent QA. |
