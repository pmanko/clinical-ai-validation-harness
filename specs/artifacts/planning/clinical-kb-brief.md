...
**A Tier-1 demo-grade precursor already ships** (not this service): `targets/med-agent-hub/server/kb.py` — an in-process SQLite **FTS5/BM25 + keyword fallback** over an openly-licensed seed (`kb_data/corpus.jsonl`: WHO general-clinical + HIV/ART, plus OCL/CIEL + OpenMRS data-model context), K=3, abstains on no match, KB facts inline-attributed and kept out of the integer-citation array — consumed by the med-agent-hub team's `kb_search` tool. It deliberately does **not** yet implement this brief's F009 features: REST + MCP (FR-009.1/.2), hybrid BM25+dense+RRF + cross-encoder rerank (FR-009.3/.4), `source_url+version+anchor` page-anchor provenance (FR-009.5), or the PHI-free contextualization worker (FR-009.8). Those are this brief's scope. See `targets/med-agent-hub/server/kb_data/README.md`, `kb-scope.md`, and `archive/med-agent-team-poc-roadmap.md` §3.
