...
| Deterministic local checks | Pass on the committed companion trees: 416 hub tests; 182 ESM tests plus lint/build; 88 Java tests through the complete Maven reactor; and 647 parent tests with 35 expected skips and 3 deselections. The parent suite required normal macOS process/semaphore permissions for its existing SQLMesh and Playwright tests. |
...
| Candidate diagnostic | Complete on run `2ae78e95-bb9b-4db7-b0b4-9b82aa187d49`: all 24 E4B/12B cells transported, but the deterministic audit found 14 blockers (nine non-terminal Answer validations, three capped-grounding unchecked cases, and two unsupported E4B citation sets). It also exposed two In-Depth exact-context overflows. This run is retained as a failed diagnostic baseline and is not eligible for judging or publication under the approved clean-audit rule. |
| Remaining proof | Deploy hub `7db9ff7`, run focused live repros for long tables and In-Depth fitting, then rerun the 24-cell candidate. If its deterministic audit is clean, independently judge, inspect, and publish it. Refresh exact-head UI proof afterward; performance tuning remains deferred. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G21 Evaluation | In progress | Diagnostic run `2ae78e95-bb9b-4db7-b0b4-9b82aa187d49` completed 24/24 but failed its deterministic audit with 14 blockers and was not judged or published. Hub `7db9ff7` remediates the cross-model stage-budget and capped-grounding defects; focused live proof and a fresh candidate remain required. |
