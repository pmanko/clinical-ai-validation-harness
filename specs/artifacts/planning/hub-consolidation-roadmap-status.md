...
|---|---|---|
| med-agent-hub | `7db9ff7` | All prior temporal, evidence, review, and nested-envelope remediation plus exact stage-local context fitting for In-Depth synthesis/review/retry, structured `insufficient_context` outcomes, complete bounded citation-grounding batches, and block-scoped table citation usage. The complete ledger still owns temporal facts, mappings, and deterministic ranking; 416 tests pass and the final independent re-review has no P0-P3 finding. |
...
| Check | Result |
|---|---|
| Canonical local startup | Prior exact-head proof passed on hub `1307f0b`; hub `7db9ff7` now requires the same canonical rerun before G19 is current again. The previous proof remains diagnostic evidence, not acceptance evidence for the new head. |
...
| Deterministic local checks | Pass on the committed companion trees: 418 hub tests; 510 Querystore tests with two optional model-dependent skips; 182 ESM tests plus lint/build; 88 ChartSearchAI tests through the complete Maven reactor; and 665 parent tests with 35 expected skips and 3 deselections. The parent suite required normal macOS process/semaphore permissions for its existing SQLMesh and Playwright tests. |
| Stage observability | Pass at hub `32ac9ea`: every configured stage records elapsed time plus completed/failed/cancelled outcome; cancellation and context-source failures reach the normal trace; report aggregates disclose completed, failed, cancelled, observed, expected, and per-status duration averages. The next run includes one human-readable `team-med-checked` arm. |
| Reindex ownership | Pass at Querystore `98f16da`: only one queued/running generation per resource type can receive `202`; duplicate same-type requests receive `409`, making the accepted generation attributable. The stable helper also holds a local lock, waits for a settled new generation, and fails explicitly on stale-extra drift. |
| Product Answer contract | Pass in code at hub `7db9ff7`: product profiles always apply the hub-owned strict `chart_answer` schema, including when a caller supplies a conflicting `response_format`; raw low-level legs remain caller-controlled. Exact stage accounting now covers Answer plus every In-Depth subcall, and nested table refs remain scoped to their cells. ChartSearchAI `550acd0` preserves the full hub wire for synchronous clients. |
...
| Live multi-turn/preemption | Pending committed-head rerun: prior UI proofs passed, but the current hub is now `7db9ff7`. |
...
| Source indexing | In progress after restoring the intended date-transplanted corpus. A fresh full Querystore bootstrap is running from empty progress state. The stable maintenance helper now force-resyncs each populated resource type and waits for a new indexing generation rather than accepting a prior completed cursor. |
...
| Evaluation provenance | In progress: preflight now compares the exact resource UUID/date ledger for every selected patient, and the obsolete Java-session fixture capture path is being replaced by the hub's current Querystore-plus-serializer path. This prevents a run from starting when the model context and judge fixture differ. |
| Next focused run | Prepare `hub-profile-team-focus`: six temporal/table/abstention scenarios across `single-e4b-checked`, `single-12b-checked`, and one `team-med-checked` arm (18 cells). The dashboard and static report expose human-readable team composition and per-stage timing with observed/expected coverage. Run only after corpus alignment and deterministic preflight pass; judge and publish only after the deterministic audit is clean. |
| Remaining proof | Finish the corpus rebuild, refresh fixtures through the hub-owned source path, run the focused 18-cell comparison, and inspect its deterministic audit. If clean, independently judge, inspect, and publish it. Refresh exact-head UI/video proof afterward; performance tuning remains deferred. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G09 Context quality | In progress | Prior verifier passed 48/48 required records on hub `1307f0b`; stage-local selection changed in `7db9ff7`, so exact-head evidence must be regenerated. |
...
| G19 Local setup | In progress | Prior exact-head proof passed on hub `1307f0b`; canonical startup and relay/hydration proof must be regenerated for `7db9ff7`. |
...
| G21 Evaluation | In progress | Diagnostic run `2ae78e95-bb9b-4db7-b0b4-9b82aa187d49` failed deterministic QA. Run `65839998-5fd5-4041-be76-0e626d7b4e96` is invalid because live and fixture date eras differed. Exact ledger alignment is now a preflight blocker; the next eligible comparison is the 18-cell single-versus-team set. |
