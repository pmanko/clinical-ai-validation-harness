...
| Product Answer contract | Pass in code at hub `4f8ea2f`: product profiles always apply the hub-owned strict `chart_answer` schema, including when a caller supplies a conflicting `response_format`; raw low-level legs remain caller-controlled. Exact stage accounting now covers Answer plus every In-Depth subcall, and nested table refs remain scoped to their cells. ChartSearchAI `9930139` preserves the full hub wire for synchronous clients. |
...
| Next focused run | `hub-profile-team-focus` is preflight-green: the two selected patients match refreshed live fixtures exactly (320 and 365 records), source authentication and the relay pass, and the parent suite is 694 passed / 35 expected skips / 4 deselections. The set contains six temporal/table/abstention scenarios across `single-e4b-checked`, `single-12b-checked`, and one `team-med-checked` arm (18 cells). Start the run from this committed checkpoint; judge and publish only after its deterministic audit is clean. |
