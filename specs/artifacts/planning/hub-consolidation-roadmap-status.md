...
|---|---|---|
| med-agent-hub | `7fd5bc7` | All prior temporal, evidence, review, and nested-envelope remediation plus exact stage-local context fitting for Answer review/retry and In-Depth synthesis/review/retry, structured `insufficient_context` outcomes, complete bounded citation-grounding batches, type-confirmed repair or withholding of malformed product tables, canonical mapping-fact grounding, block-scoped table citation usage, and one canonical stage-timing trace schema. Stage durations are closed before public SSE yields so client backpressure is excluded; 429 tests pass. |
...
| Check | Result |
|---|---|
| Canonical local startup | Prior exact-head proof passed on hub `1307f0b`; hub `7fd5bc7` now requires the same canonical rerun before G19 is current again. The previous proof remains diagnostic evidence, not acceptance evidence for the new head. |
...
| Stage observability | Pass at hub `7fd5bc7`: every configured stage records elapsed time plus completed/failed/cancelled outcome in one schema; cancellation and context-source failures reach the normal trace; timings close before SSE delivery; dashboard and report tests render status plus observed/expected coverage. The next run includes one human-readable `team-med-checked` arm. |
...
| Product Answer contract | Pass in code at hub `7fd5bc7`: product profiles always apply the hub-owned strict `chart_answer` schema, including when a caller supplies a conflicting `response_format`; raw low-level legs remain caller-controlled. Dynamic table keys are checked after generation, with only type-confirmed date/weight flattening repaired and ambiguous blocks withheld. Exact stage accounting now covers Answer plus every In-Depth subcall, and nested table refs remain scoped to their cells. ChartSearchAI `9930139` preserves the full hub wire for synchronous clients. |
...
| Live multi-turn/preemption | Pending committed-head rerun: prior UI proofs passed, but the current hub is now `7fd5bc7`. |
...
| Focused diagnostic remediation | Pass in code at hub `7fd5bc7`: malformed dynamic tables are repaired only for type-confirmed date/weight rows and otherwise withheld; Answer review and correction recheck each fit the evidence ledger independently; grounding never silently drops a cited source and only bypasses semantic review for canonical mapping date/value facts. The hub suite passes 429 tests, the focused regression suite passes 95 tests, and independent review found no remaining actionable issue. Live 18-cell rerun pending. |
| Remaining proof | Remediate the focused diagnostic with red-first table-contract, grounding, audit, and reviewer-budget tests; rerun the affected cells and then the complete 18-cell comparison. Only a deterministically clean run advances to independent judging and candidate publication. Refresh exact-head UI/video proof afterward; performance tuning remains deferred. Final DIGI-UW/code-qa and release hygiene remain pending. |
