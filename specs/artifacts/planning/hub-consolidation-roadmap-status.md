...
| Remaining proof | Publish the current landing video, run the exact-head live gate matrix, refresh all paired PR descriptions, and run final DIGI-UW/code-qa plus release-hygiene checks. The identity-bound relay/hydration proof now passes on the committed heads; E2B/E4B co-residency is separately observed rather than inferred. All 496 hub tests pass. E4B and medical-team structural-output remediation remain separate workstreams, strict-window interpretation remains deferred to the typed temporal-query workstream, and performance tuning remains deferred. |
...
| M3 Product/local proof | In progress | Exact-head startup, relay/hydration, small-model co-residency, and the replacement expanded E4B video now pass. The final `RUN_E2E=1` adapter and focused live gate matrix remains before completion. |
