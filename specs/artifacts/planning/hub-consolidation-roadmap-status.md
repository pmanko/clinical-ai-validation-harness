...
|---|---|---|
| med-agent-hub | `4f8ea2f` | All prior temporal, evidence, review, and nested-envelope remediation plus exact stage-local context fitting for In-Depth synthesis/review/retry, structured `insufficient_context` outcomes, complete bounded citation-grounding batches, block-scoped table citation usage, and one canonical stage-timing trace schema. Stage durations are closed before public SSE yields so client backpressure is excluded; 419 tests pass. |
...
| Check | Result |
|---|---|
| Canonical local startup | Prior exact-head proof passed on hub `1307f0b`; hub `4f8ea2f` now requires the same canonical rerun before G19 is current again. The previous proof remains diagnostic evidence, not acceptance evidence for the new head. |
...
| Deterministic local checks | Pass on the committed companion trees: 419 hub tests; 512 Querystore tests with two optional model-dependent skips; 182 ESM tests plus lint/build; 88 ChartSearchAI tests through the complete Maven reactor; and 688 parent tests with 35 expected skips and 4 deselections. The real MariaDB portable-dump round-trip and two Chromium timing-UX tests also pass. The parent suite required normal macOS process/semaphore permissions for its existing SQLMesh and Playwright tests. |
| Stage observability | Pass at hub `4f8ea2f`: every configured stage records elapsed time plus completed/failed/cancelled outcome in one schema; cancellation and context-source failures reach the normal trace; timings close before SSE delivery; dashboard and report tests render status plus observed/expected coverage. The next run includes one human-readable `team-med-checked` arm. |
...
| Live multi-turn/preemption | Pending committed-head rerun: prior UI proofs passed, but the current hub is now `4f8ea2f`. |
...
| G09 Context quality | In progress | Prior verifier passed 48/48 required records on hub `1307f0b`; stage-local selection changed before `4f8ea2f`, so exact-head evidence must be regenerated. |
...
| G19 Local setup | In progress | Prior exact-head proof passed on hub `1307f0b`; canonical startup and relay/hydration proof must be regenerated for `4f8ea2f`. |
