...
|---|---|---|
| med-agent-hub | `00f2d8c` | Temporal/date-value enforcement, final grounding lifecycle, context selection traceability, and test isolation hardened; 311 tests pass |
...
| Check | Result |
|---|---|
| Canonical local startup | Implemented; proof pending: `scripts/chartsearchai-local.sh` now delegates hub deployment to the credential-aware `make med-agent-hub-up`, verifies profile availability/default integrity, warms E4B, and runs a fast-Answer relay/persistence probe. G19 remains pending until that probe is regenerated on clean committed heads. |
...
| Warm performance observation | Pending recollection: the previous identity-bound distribution described hub `de8614e`; G20 must be recollected against committed hub `00f2d8c` and its exact labeled image. The criterion remains relative with `fixed_latency_threshold: null`. |
| Live multi-turn/preemption | Pending committed-head rerun: the focused proof on `de8614e` observed a positive Q2 cancellation marker with `router_lock_released=true` and a completed Q3 trace, but the final hub behavior is now `00f2d8c`. |
...
| G20 Performance | In progress | The collector binds deployed image revision plus source/config/model identity; the warm distribution must be recollected on hub `00f2d8c` without an arbitrary absolute latency threshold. |
