...
Amendment result: **Pass** at med-agent-hub `00ae4c4`. The hub emits one unversioned internal
`temporal_facts` object, traces require an exact hub commit, 385 hub tests pass, 34 focused parent
...
|---|---|---|
| med-agent-hub | `00ae4c4` | Temporal/date-value enforcement, final grounding lifecycle, context selection traceability, network-independent unprivileged runtime health, explicit encounter evidence, product citation-scope checks, Git-owned temporal-facts provenance, claim-local In-Depth concept/direction binding, one normalized knowledge-evidence path with deterministic multi-turn recall and complete provenance, and enforce-only repair of uniquely bound truncated numeric citations; 385 tests pass and independent re-review is clean. |
...
| Check | Result |
|---|---|
| Canonical local startup | Implemented; repaired-head proof pending: `scripts/chartsearchai-local.sh` delegates hub deployment to `make med-agent-hub-up`, verifies profile availability/default integrity, warms E4B, and runs a full reviewed-lifecycle relay/persistence probe. The first committed-head run produced a correct checked/grounded Answer in 2.507 seconds, then failed closed at In-Depth and exposed two false-positive temporal bindings. The next run on `ae6be43` confirmed those temporal fixes, then correctly withheld all three claims because each combined a patient record with unsupported outside interpretation. Hub `36284dd` aligned writer, reviewer, and final grounding contracts and normalized KB evidence; its live run then produced one correct numeric claim with a truncated trailing citation. Hub `00ae4c4` adds an enforce-only repair when concept, date, value, and exact non-compound unit identify one record; ambiguous, wrong-unit, uncited, existing-citation, off, and warn cases cannot be silently rewritten. The canonical probe must now produce `indepth_done` before G19 passes. The hub runtime no longer contacts a package mirror for curl; Compose inherits its Python health check, maps a non-root host UID/GID for trace writes, rejects root launchers, and proves the actual bind mount is writable. |
...
| Warm performance observation | Pending recollection: the previous identity-bound distribution described hub `de8614e`; the first canonical run observed a 2.507-second warm E4B `answer_done`, but G20 requires a distribution against committed hub `00ae4c4` and its exact labeled image. The criterion remains relative with `fixed_latency_threshold: null`. |
| Live multi-turn/preemption | Pending committed-head rerun: the focused proof on `de8614e` observed a positive Q2 cancellation marker with `router_lock_released=true` and a completed Q3 trace, but the current hub is now `00ae4c4`. |
...
| Remaining proof | Push the parent pin for hub `00ae4c4`, deploy the exact clean heads, require a complete `indepth_done` lifecycle, regenerate G09/G18-G20 and video evidence, then run, independently judge, inspect, and publish the 24-cell candidate. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G09 Context quality | In progress | The selector records an inclusion reason for every selected record and uses token-boundary identifier matching. The stable verifier passed 12 cases, 48/48 required records, and 1.0 recall on `d7bd52e`; it must be regenerated on final hub `00ae4c4` so artifact identity matches the release head. Deterministic KB tests additionally protect current-topic hits and reserve at most one prior-answer expansion hit. |
...
| G20 Performance | In progress | The collector binds deployed image revision plus source/config/model identity; the warm distribution must be recollected on hub `00ae4c4` without an arbitrary absolute latency threshold. The single 2.507-second warm observation is encouraging but is not the required distribution. |
