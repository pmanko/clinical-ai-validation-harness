...
| Check | Result |
|---|---|
| Canonical local startup | Pass on hub `1307f0b`: `make chartsearchai-local` verified profile/default integrity and completed `answer_done -> answer_validation -> indepth_pending -> indepth_done -> done` with a checked Answer, complete In-Depth, five terminal references, one message/audit row, and byte-identical final stream versus hydrated history. The identity-bound proof records the exact hub, parent, ChartSearchAI, ESM, configuration, image, and served-asset revisions. Warmup reached `answer_done` in 0.897 seconds; the full OpenMRS relay reached it in 28.988 seconds and completed in 62.550 seconds. These are observations, not hard quality or latency thresholds. G19 proves terminal relay, validation, evidence, same-row persistence, and hydration integrity while G21 separately measures E4B clinical quality. |
...
| Remaining proof | Run the focused explicitly requested context counterexample, then regenerate G18/G20 and video evidence; run, independently judge, inspect, and publish the 24-cell candidate. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G09 Context quality | Pass | Exact-head stable verifier passed 12 cells with 48/48 required records, zero missing, 1.0 recall, deterministic inclusion/exclusion reasons, and exact tokenizer-backed budgets on hub `1307f0b` |
...
| G19 Local setup | Pass | Canonical startup on hub `1307f0b` produced an identity-bound relay proof with a terminal checked Answer, complete In-Depth, same-row persistence, five terminal references, and byte-identical stream/hydration envelopes. The gate also accepts reasoned needs-review safety outcomes when the lifecycle is internally consistent; it does not score one E4B sample. |
