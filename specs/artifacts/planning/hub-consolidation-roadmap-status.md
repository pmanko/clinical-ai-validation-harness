...
| Remaining proof | Add a generic deterministic strict-window validator over the full dated evidence ledger, covering out-of-window inclusions and qualifying in-window omissions without scenario-specific prompt tuning. Re-run only targeted repro cells first, then the 12-scenario candidate if the fix is clean. E4B and medical-team structural-output remediation remain separate workstreams. Refresh exact-head UI/video proof afterward; performance tuning remains deferred. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| M3 Product/local proof | In progress | Stage observability, product profiles, final grounding, temporal enforcement, and exact-head product runs are proven. Current-head OpenMRS UI/video, cancellation, source-quality refresh, and canonical local-startup proof remain before completion. |
| M4 Evaluation and release | In progress | The fresh 12B candidate is independently judged and published, but the judge exposed two strict-window failures outside the implemented deterministic audit. Generic window-gate remediation, targeted/candidate reruns, final independent QA, and User Release Signoff D remain. |
