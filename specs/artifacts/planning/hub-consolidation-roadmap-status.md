...
| G19 Local setup | In progress | Stable startup discovers the sole available hub default and configures only the hub endpoint. Its current-head relay probe correctly failed on the missing In-Depth pending phase when validation withheld the tail; hub `2422f91` fixes and tests that event contract. Exact clean-head redeployment and proof regeneration remain. |
