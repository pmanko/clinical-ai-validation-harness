...
|---|---|---|
| med-agent-hub | `1307f0b` | Temporal/date-value enforcement, final grounding lifecycle, context selection traceability, network-independent unprivileged runtime health, explicit encounter evidence, product citation-scope checks, Git-owned temporal-facts provenance, claim-local In-Depth concept/direction and paired-series value binding, one normalized knowledge-evidence path with deterministic multi-turn recall and complete provenance, enforce-only citation repair/canonicalization, bounded In-Depth review that preserves accepted claims and reports lifecycle-wide edit metadata, small-model claim-shape guidance that keeps factual point lookups narrow without suppressing requested supported context, and canonical nested terminal In-Depth envelopes that preserve the direct Answer; 406 tests pass and independent re-review is clean. |
...
| Check | Result |
|---|---|
| Canonical local startup | Passed on hub `00ae4c4`: `scripts/chartsearchai-local.sh` verified profile/default integrity and completed `answer_done -> answer_validation -> indepth_pending -> indepth_done -> done`, complete In-Depth, three grounded references, and byte-identical stream/hydration envelopes. The Answer appeared at 25.191 seconds, completion was 70.720 seconds, and the separate warm hub probe was 1.565 seconds. Follow-up traces exposed conservative paired-series binding, destructive whole-section regeneration, E4B drift from a factual lookup into unrelated multi-record claims, and terminal In-Depth fields that overwrote or differed from the direct Answer envelope; those issues are fixed or narrowed and independently reviewed at hub `1307f0b`. G19 now proves terminal relay, validation, evidence, same-row persistence, and hydration integrity while G21 separately measures E4B answer quality. Exact-head canonical replay is pending before G19 returns to Pass. |
...
| Atomic In-Depth evidence | Pass in code: `indepth_done`/`indepth_error` carry the full final envelope with canonical nested `inDepth`; the hub no longer emits colliding flattened `answer`/`status` fields, while Java and ESM retain input compatibility for historical flattened events. Java persists final references and verdicts with the terminal In-Depth state, and EOF coverage requires them. Exact-head live replay remains pending. |
| Warm performance observation | In progress: exact-head hub warmup observed 1.565-second `answer_done`; the full OpenMRS relay observed 25.191-second `answer_done` and 70.720-second completion. G20 still requires the identity-bound warm distribution and direct-router comparison; the criterion remains relative with `fixed_latency_threshold: null`. |
| Live multi-turn/preemption | Pending committed-head rerun: the focused proof on `de8614e` observed a positive Q2 cancellation marker with `router_lock_released=true` and a completed Q3 trace, but the current hub is now `1307f0b`. |
...
| Remaining proof | Replay G09 and the canonical lifecycle on hub `1307f0b`, including a focused explicitly requested context counterexample, then regenerate G18-G20 and video evidence; run, independently judge, inspect, and publish the 24-cell candidate. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G09 Context quality | In progress | The stable verifier passed 12 cells with 48/48 required records, zero missing, and 1.0 recall on hub `00ae4c4`; the artifact must be regenerated on current hub `1307f0b` so its code hash matches. |
...
| G19 Local setup | In progress | Canonical startup passed on hub `00ae4c4`; exact-head replay is required on current hub `1307f0b`. This gate accepts checked, edited, or reasoned needs-review clinical states only when the staged wire, terminal evidence, persistence, hydration, and runtime identity are internally consistent; it does not score one E4B sample. |
| G20 Performance | In progress | The collector binds deployed image revision plus source/config/model identity; the warm distribution must be recollected on hub `1307f0b` without an arbitrary absolute latency threshold. |
| G21 Evaluation | In progress | The exact 24-cell E4B/12B candidate, deterministic audit, independent-actor provenance, recomputed consensus, and per-cell baseline contract are executable; this gate, not G19, measures checked/edited/needs-review rates and clinical quality. Source indexing is complete and exact-head deployment/preflight are next. |
