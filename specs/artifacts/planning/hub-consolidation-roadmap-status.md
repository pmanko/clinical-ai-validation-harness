...
| Live multi-turn/preemption | Fail on exact parent `685d838` and hub `6068d2d`. The live multi-turn flow passed. The preemption product behavior also completed: the replacement question was accepted and answered, and interrupted work reached a terminal failed state. The final telemetry assertion failed because `StageEngine` samples the shared global router lock after the cancelled request unwinds; the replacement request may already own that lock, so `router_lock_released=false` does not identify which request owns it. Replace this race-prone global snapshot with request-scoped release evidence, retain the deterministic cancellation contracts, and rerun only the focused preemption proof. |
| Full stage-refactor matrix | Partial on the current exact heads. The consolidation matrix now passes G07 after the QueryStore correction, but G18 fails on the request-ownership telemetry race above. G03, G09, G19, G21, G23, and consequently G24 remain pending; G20 is explicitly deferred. Older all-green stage-refactor results remain historical evidence and do not override this exact-head matrix. |
...
| G18 Multi-turn and cancellation | Fail | On exact parent `685d838` with hub `6068d2d`, the multi-turn flow passed and the preemption UI behavior completed, but cancellation telemetry sampled the shared lock after the replacement request could acquire it and reported `router_lock_released=false`. Request-scoped release evidence and a focused live rerun are required. |
