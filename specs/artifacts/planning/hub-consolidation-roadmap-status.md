...
| Live multi-turn/preemption | Pass on parent `0db0a2c`, hub `af30dc5`, ChartSearchAI `7b91ed1`, and ESM `3ac24b3`. A first current-head run exposed a real follow-up failure when llama-router dropped one exact token-count connection. Hub `af30dc5` now retries that idempotent transport operation once, while persistent failure, HTTP errors, and cancellation remain fail-closed; the red-first cases and all 502 hub tests pass. After redeployment, `chartsearchai-e4b-multiturn-trivial` passed in 1.3 minutes and carried the first Answer's exact date into the follow-up; its measured Answer times were 10.6 and 13.7 seconds. `chartsearchai-preempt` passed in 2.8 minutes, proving the replacement turn was answered and the interrupted work reached a terminal state with released router capacity. |
...
| Remaining proof | Scout-judge and publish the completed small-model comparison, refresh all paired PR descriptions, and finish final DIGI-UW/code-qa plus release-hygiene checks. The exact-head relay/hydration, E2B/E4B co-residency, context-quality, multi-turn, preemption, landing video, and all 502 hub tests now pass. The managed independent-review runner failed to return three times, while the external CLI fallback was blocked by private-workspace policy; G23 therefore remains pending rather than being self-certified. Medical-team structural-output remediation, typed temporal-query work, and performance/cache tuning remain separate workstreams. |
...
| G18 Multi-turn and cancellation | Pass | On the deployed `af30dc5` hub, the two-turn E4B proof passed in 1.3 minutes with exact date carry-forward, and the positive preemption proof passed in 2.8 minutes with a completed replacement turn and released router capacity. The transient token-count disconnect found by the first attempt is now a red-first regression test. |
| G19 Local setup | Pass | Parent `0db0a2c` deployed through the canonical startup, configures only med-agent-hub, discovers `single-e4b-checked`, proves the real staged relay and byte-identical hydration, and records exact runtime/artifact identity. A separate test-only router proof invokes E2B and E4B sequentially and observes both loaded afterward without adding product warmup. |
