...
|---|---|---|
| med-agent-hub | `98da087` | Temporal/date-value enforcement, final grounding lifecycle, context selection traceability, network-independent unprivileged runtime health, explicit encounter evidence, product citation-scope checks, Git-owned temporal-facts provenance, claim-local In-Depth concept/direction and paired-series value binding, one normalized knowledge-evidence path with deterministic multi-turn recall and complete provenance, enforce-only citation repair/canonicalization, bounded In-Depth review that preserves accepted claims and reports lifecycle-wide edit metadata, and small-model claim-shape guidance that keeps factual point lookups narrow without suppressing requested supported context; 406 tests pass and independent re-review is clean. |
...
| Check | Result |
|---|---|
| Canonical local startup | Passed on hub `00ae4c4`: `scripts/chartsearchai-local.sh` verified profile/default integrity and completed `answer_done -> answer_validation -> indepth_pending -> indepth_done -> done`, complete In-Depth, three grounded references, and byte-identical stream/hydration envelopes. The Answer appeared at 25.191 seconds, completion was 70.720 seconds, and the separate warm hub probe was 1.565 seconds. Follow-up traces exposed conservative paired-series binding, destructive whole-section regeneration, and E4B drift from a factual lookup into unrelated multi-record claims; those issues are fixed or narrowed and independently reviewed at hub `98da087`. Exact-head canonical replay is pending before G19 returns to Pass. The runtime remains unprivileged, network-independent for health, and verifies the mounted trace path. |
...
| Live multi-turn/preemption | Pending committed-head rerun: the focused proof on `de8614e` observed a positive Q2 cancellation marker with `router_lock_released=true` and a completed Q3 trace, but the current hub is now `98da087`. |
...
| Remaining proof | Replay G09 and the canonical lifecycle on hub `98da087`, including a focused explicitly requested context counterexample, then regenerate G18-G20 and video evidence; run, independently judge, inspect, and publish the 24-cell candidate. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G09 Context quality | In progress | The stable verifier passed 12 cells with 48/48 required records, zero missing, and 1.0 recall on hub `00ae4c4`; the artifact must be regenerated on current hub `98da087` so its code hash matches. |
...
| G19 Local setup | In progress | Canonical startup passed on hub `00ae4c4`; exact-head replay is required on current hub `98da087` after the paired-series and In-Depth reliability fixes. |
| G20 Performance | In progress | The collector binds deployed image revision plus source/config/model identity; the warm distribution must be recollected on hub `98da087` without an arbitrary absolute latency threshold. |
