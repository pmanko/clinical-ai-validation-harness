...
| Check | Result |
|---|---|
| Canonical local startup | **Pass** on exact product heads: `scripts/chartsearchai-local.sh` deployed hub `00ae4c4`, ChartSearchAI `550acd0`, and ESM `b3ad02d`; verified profile/default integrity; warmed E4B; and completed the OpenMRS relay/persistence probe with `answer_done -> answer_validation -> indepth_pending -> indepth_done -> done`. The Answer appeared at 25.191 seconds, final completion was 70.720 seconds, In-Depth was complete, three references were grounded, and the hydrated envelope SHA-256 exactly matched the streamed final envelope. The separate warm hub probe returned `answer_done` in 1.565 seconds. Proof: `artifacts/chartsearchai-local/relay-probe.json`. Earlier failed probes exposed and drove remediation for temporal false positives, unsupported interpretation, and a truncated citation. The hub runtime is unprivileged, uses a network-independent Python health check, and proves the mounted trace path is writable. |
...
| Remaining proof | Resolve or explicitly disposition the paired same-unit multi-series binding false positive observed when a correct systolic/diastolic claim was conservatively removed; regenerate G18-G20 and video evidence; then run, independently judge, inspect, and publish the 24-cell candidate. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G09 Context quality | Pass | `scripts/verify-context-quality.py` passed 12 cells with 48/48 required records, zero missing, and 1.0 recall against hub code hash `09268b33e6bd8ba97326a5a50426310d7cd8248646f6e33787c2b8c21f30c340`. The stable script now prefers the parent gate environment and the artifact is `artifacts/roadmap/gates/G09-context-quality.json`. Deterministic KB tests protect current-topic hits and reserve at most one prior-answer expansion hit. |
...
| G19 Local setup | Pass | Canonical startup on exact clean product heads produced the required event order, checked/grounded Answer, complete In-Depth, byte-identical stream/hydration envelopes, mounted OMOD identity, and complete served ESM/import-map provenance in `artifacts/chartsearchai-local/relay-probe.json`. |
