...
| Remaining proof | Run the one-cell `hub-profile-12b-appointment-smoke` on exact committed heads and require a zero-blocker deterministic audit. If clean, rerun the 12B-only 12-scenario quality baseline from one exact head, independently judge it, and publish the report with explicit scope. E4B and medical-team structural-output remediation remain separate workstreams. Refresh exact-head UI/video proof afterward; performance tuning remains deferred. Final DIGI-UW/code-qa and release hygiene remain pending. |
