...
| Current integration rehearsal | Pass on companion heads hub `fc62811`, ChartSearchAI `7b91ed1`, ESM `3ac24b3`, and Querystore `dd2da77`: 496 hub tests; the complete ChartSearchAI Maven reactor; 191 ESM tests plus typecheck, lint, and production build; 512 Querystore tests with two optional-model skips; and 788 parent tests with 35 expected skips and four deselections. The restricted parent run failed only where macOS blocked local sockets, semaphores, and Chromium; the identical suite passed with normal test permissions. |
...
| Remaining proof | Commit and push the parent integration heads, refresh the identity-bound relay artifact, then run final DIGI-UW/code-qa and release-hygiene checks. The canonical local relay probe exposed a missing `indepth_pending` event when a final Answer required review; red-first lifecycle tests now cover reviewed, unreviewed, unsupported, unavailable, and non-substantive terminal paths, and all 496 hub tests pass. The corrected live multi-turn and preemption proofs pass; the latter accepts both completed and safety-withheld baseline In-Depth as healthy terminal states while still requiring an active later In-Depth phase, positive cancellation record, released router slot, and completed replacement turn. E4B and medical-team structural-output remediation remain separate workstreams, strict-window interpretation remains deferred to the typed temporal-query workstream, and performance tuning remains deferred. |
...
| G18 Multi-turn and cancellation | Pass | The live E4B multi-turn proof passed in the full matrix, and the corrected focused preemption proof passed in 3.3 minutes with a positive hub cancellation record, released router slot, terminal preempted row, and completed replacement turn. |
| G19 Local setup | In progress | Stable startup discovers the sole available hub default and configures only the hub endpoint. A separate test-only router proof now invokes E2B and E4B sequentially and observes both as loaded afterward; this does not add product warmup. Exact clean-head relay redeployment and proof regeneration remain. |
...
| G23 Independent QA | Pending | Independent correctness and all five DIGI-UW/code-qa review contracts pass with zero code/test blockers on the committed companion heads. The final ignored `artifacts/roadmap/code-qa/result.json` must still be generated and hash-bound to the parent commit and updated submodule pins. |
