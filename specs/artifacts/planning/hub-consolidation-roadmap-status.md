...
|---|---|---|
| med-agent-hub | `77cfeca` | Intermediate tested checkpoint: temporal/date-value enforcement, final grounding lifecycle, context selection traceability, network-independent unprivileged runtime health, explicit encounter evidence, and product citation-scope checks; 324 tests pass. Independent re-review still requires removal of manual temporal schema-version proliferation plus sparse-visit and post-gate citation-order fixes before this work can pass. |
...
| Warm performance observation | Pending recollection: the previous identity-bound distribution described hub `de8614e`; G20 must be recollected against committed hub `77cfeca` and its exact labeled image. The criterion remains relative with `fixed_latency_threshold: null`. |
| Live multi-turn/preemption | Pending committed-head rerun: the focused proof on `de8614e` observed a positive Q2 cancellation marker with `router_lock_released=true` and a completed Q3 trace, but the current checkpoint is now `77cfeca`. |
...
| G20 Performance | In progress | The collector binds deployed image revision plus source/config/model identity; the warm distribution must be recollected on hub `77cfeca` without an arbitrary absolute latency threshold. |
