...
| Check | Result |
|---|---|
| Canonical local startup | Prior exact-head proof passed on hub `1307f0b`; hub `021e305` now requires the same canonical rerun before G19 is current again. The previous proof remains diagnostic evidence, not acceptance evidence for the new head. |
...
| Remaining proof | Deploy `021e305` and run `hub-profile-appointment-smoke` to verify the two infrastructure failures on exact committed heads. Then choose the next publish candidate arms from the measured product behavior rather than weakening deterministic checks: 12B remains the quality baseline; E4B and the medical-team profile require model/prompt iteration for the failed cells. Refresh exact-head UI/video proof afterward; performance tuning remains deferred. Final DIGI-UW/code-qa and release hygiene remain pending. |
