...
| G07 Source independence | Pending live refresh | Inline, optional Querystore, static KB, and mock alternate adapters share one normalized source contract, and the hub starts without Querystore. The external adapter now mirrors upstream QueryStore read behavior instead of reconciling on each request; hub transport checks still reject malformed, duplicated, truncated, or count-inconsistent pages. Index readiness is verified and repaired through QueryStore's operational endpoints. Exact deployed-OMOD identity, live source timing, and final Answer evidence must be refreshed after the thin-adapter commit. |
