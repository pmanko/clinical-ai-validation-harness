...
| Live multi-turn/preemption | Pass on exact current heads parent `14aa1fd`, hub `af30dc5`, ChartSearchAI `7b91ed1`, and ESM `3ac24b3`. The earlier token-count disconnect remains covered by the one-retry, cancellation-aware transport fix and red-first tests. The final `RUN_E2E=1` acceptance run passed both real Chromium flows serially: `chartsearchai-e4b-multiturn-trivial` in 3.8 minutes and `chartsearchai-preempt` in 4.2 minutes. The follow-up retained prior clinical context; the replacement turn completed; interrupted work reached a terminal state; and router capacity was released. These variable local durations are observations, not an absolute performance gate. |
| Full stage-refactor matrix | Pass with `RUN_E2E=1`: all 14 architecture gates are green on the current source heads, including byte-exact raw-leg checks, the complete ESM suite, the full ChartSearchAI Maven reactor, and fresh live multi-turn/preemption checks. The broader exact-head consolidation matrix passes G01-G19, with G20 explicitly deferred; G21, G23, and consequently G24 remain non-green for the disclosed evaluation and independent-review reasons below. |
...
| Exact-head context quality | Pass at hub `7bb9371`: the stable verifier replayed 12 labeled E4B/12B cases through the real hub context-preparation path and llama.cpp tokenizer with all 48 required sources available. The input limit is now a ceiling rather than a packing target: mandatory safety evidence, exact matches, a bounded 32-record clinical core that prioritizes active conditions, and meaningful normalized overlap are eligible; other records are traced as `zero_relevance`. Exact inputs fell from the prior 19,632-20,467 range to 13,455-17,493 under the unchanged 20,480 limit, leaving 2,987-7,025 tokens unused; selected records fell from a 211.3 average to 51.2. No case needed a token-budget exclusion. Selected records render in canonical chart order, while temporal facts and drug-safety preparation are tested against the complete ledger. The hub suite passes 531 tests. |
...
| Complete patient-chart transport | Pass in code at hub `ef403cc` and Querystore `4a081e3`: full-chart projection is strict while ranked search retains tolerant lazy indexing; per-type serialization, embedding, and write failures propagate; MySQL, Lucene, and Elasticsearch fail closed on unreadable or disappeared known stores; Elasticsearch uses uncapped PIT/`search_after`; REST pages carry a canonical typed-JSON SHA-256 snapshot identity; and the hub rejects changed snapshots, duplicate/malformed records, invalid totals, and non-complete envelopes. Hub 511, Querystore unit, MySQL integration, Elasticsearch integration, root gate/helper tests, and the seven-repository drift scan pass. The final independent review found no P0-P2 blocker. |
...
| G07 Source independence | Pending live refresh | Inline, optional Querystore, static KB, and mock alternate adapters share one normalized source contract, and the hub starts without Querystore. Unit plus MySQL and Elasticsearch integration suites now execute the complete-or-error contract, including strict projection, disappeared stores, snapshot consistency, and uncapped pagination. The strengthened live check additionally requires the exact deployed Querystore OMOD and final Answer evidence from that adapter; its post-parent-commit artifact is pending. |
...
| G09 Context quality | Pass | Current-head proof retains 48/48 required records across 12 cases with exact inputs at or below 20,480 tokens; date-only metadata cannot mask missing evidence, four contexts are full, eight selected, and every selection decision has a trace reason |
...
| G18 Multi-turn and cancellation | Pass | On exact head `14aa1fd` with hub `af30dc5`, the final live Chromium run passed the 3.8-minute two-turn E4B flow and the 4.2-minute positive preemption flow. Prior clinical context was retained, the replacement turn completed, interrupted work became terminal, and router capacity was released. The transient token-count disconnect found by an earlier attempt remains a red-first regression test. |
...
| 2026-07-16 | Treat the exact model input limit as a safety ceiling, not a context-fill objective. | The previous oversized-chart selector ranked every remaining record, including records with zero query relevance, and therefore packed prompts to the limit. Hub `7bb9371` adds deterministic eligibility before exact budget admission, preserves a bounded active-condition/recent clinical core, traces zero-relevance exclusions, stops when eligible evidence is exhausted, and retains complete-ledger temporal and drug-safety checks. The labeled exact-token gate remains 48/48, so this is a general context-supply correction rather than a benchmark-specific prompt rule. |
