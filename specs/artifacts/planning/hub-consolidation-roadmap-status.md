...
Amendment result: **Pass** at med-agent-hub `ae6be43`. The hub emits one unversioned internal
`temporal_facts` object, traces require an exact hub commit, 365 hub tests pass, 34 focused parent
...
|---|---|---|
| med-agent-hub | `ae6be43` | Temporal/date-value enforcement, final grounding lifecycle, context selection traceability, network-independent unprivileged runtime health, explicit encounter evidence, product citation-scope checks, Git-owned temporal-facts provenance, and claim-local In-Depth concept/direction binding; 365 tests pass and independent re-review is clean. |
...
| Check | Result |
|---|---|
| Canonical local startup | Implemented; repaired-head proof pending: `scripts/chartsearchai-local.sh` delegates hub deployment to `make med-agent-hub-up`, verifies profile availability/default integrity, warms E4B, and runs a full reviewed-lifecycle relay/persistence probe. The first committed-head run produced a correct checked/grounded Answer in 2.507 seconds, then correctly failed closed at In-Depth because the generated section contained uncited claims and exposed two false-positive concept bindings. Hub `ae6be43` remediates those bindings and tightens the In-Depth prompt; the canonical probe must now produce `indepth_done` before G19 passes. The hub runtime no longer contacts a package mirror for curl; Compose inherits its Python health check, maps a non-root host UID/GID for trace writes, rejects root launchers, and proves the actual bind mount is writable. |
...
| Warm performance observation | Pending recollection: the previous identity-bound distribution described hub `de8614e`; the first canonical run observed a 2.507-second warm E4B `answer_done`, but G20 requires a distribution against committed hub `ae6be43` and its exact labeled image. The criterion remains relative with `fixed_latency_threshold: null`. |
| Live multi-turn/preemption | Pending committed-head rerun: the focused proof on `de8614e` observed a positive Q2 cancellation marker with `router_lock_released=true` and a completed Q3 trace, but the current hub is now `ae6be43`. |
...
| Remaining proof | Push hub `ae6be43` and the parent pin, deploy those exact clean heads, require a complete `indepth_done` lifecycle, regenerate G09/G18-G20 and video evidence, then run, independently judge, inspect, and publish the 24-cell candidate. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G20 Performance | In progress | The collector binds deployed image revision plus source/config/model identity; the warm distribution must be recollected on hub `ae6be43` without an arbitrary absolute latency threshold. The single 2.507-second warm observation is encouraging but is not the required distribution. |
