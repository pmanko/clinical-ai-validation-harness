...
|---|---|---|
| med-agent-hub | `ea833f4` | Temporal/date-value enforcement, final grounding lifecycle, context selection traceability, test isolation, and network-independent unprivileged runtime health hardened; 315 tests pass |
...
| Warm performance observation | Pending recollection: the previous identity-bound distribution described hub `de8614e`; G20 must be recollected against committed hub `ea833f4` and its exact labeled image. The criterion remains relative with `fixed_latency_threshold: null`. |
| Live multi-turn/preemption | Pending committed-head rerun: the focused proof on `de8614e` observed a positive Q2 cancellation marker with `router_lock_released=true` and a completed Q3 trace, but the final hub behavior is now `ea833f4`. |
...
| G20 Performance | In progress | The collector binds deployed image revision plus source/config/model identity; the warm distribution must be recollected on hub `ea833f4` without an arbitrary absolute latency threshold. |
