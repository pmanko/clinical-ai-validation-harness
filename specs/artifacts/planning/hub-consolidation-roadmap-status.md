...
Local validation on these heads: parent 788 passed with 35 environment-dependent skips and four
deselections; ChartSearchAI 92 passed; ESM 191 passed plus typecheck, lint, and production build;
...
| Exact-head context quality | Pass at hub `af30dc5`: the stable verifier replayed 12 labeled cases through the real hub context-preparation path and llama.cpp tokenizer. All 48 required sources were available with zero missing; four cases retained full context and eight selected deterministically. Date-only temporal summaries no longer count as supplied clinical evidence; only selected records and fact-bearing numeric/date-observation sidecar entries qualify. The allergy safety record is selected as mandatory. Exact inputs ranged from 19,632 to 20,467 tokens under the 20,480-token limit, and every included/excluded record retained a reason. |
...
| Remaining proof | Finish the independent DIGI-UW/code-qa review set, then close release hygiene only if its exact-head result is non-blocking. Exact-head relay/hydration, E2B/E4B co-residency, context quality, multi-turn, preemption, landing video, completed judgments, published reports, paired PR heads/CI, clean pins, and all 502 hub tests pass. The latest live consolidation matrix therefore passes G01-G19, with G20 deferred. G21 remains partial by explicit scope rather than passing; the managed independent-review runner failed to return three times and private-workspace policy blocked an external CLI fallback, so G23 remains pending rather than self-certified and G24 cannot pass. Medical-team structural-output remediation, typed temporal-query work, and performance/cache tuning remain separate workstreams. |
...
| M3 Product/local proof | Complete | Exact-head startup, relay/hydration, small-model co-residency, context quality, expanded E4B video, source adapter, multi-turn, and preemption proofs pass on the current product heads. |
...
| G19 Local setup | Pass | Parent `14aa1fd` deployed through the canonical startup, configures only med-agent-hub, discovers `single-e4b-checked`, proves the real staged relay and byte-identical hydration, and records exact runtime/artifact identity for hub `af30dc5`, ChartSearchAI `7b91ed1`, and ESM `3ac24b3`. A separate test-only router proof invoked E2B and E4B sequentially and observed both loaded before and after, without eviction or product warmup. |
...
| G23 Independent QA | Pending | The official DIGI-UW/code-qa checkout is current and the deterministic evidence bundle has been regenerated from the live E4B run. Three managed independent-review attempts did not return a report, and private-workspace policy blocked an external CLI fallback. The meaningful-coverage, simplicity, spec/code, and companion reports plus exact-head result therefore remain pending; no green result has been fabricated. |
| G24 Release hygiene | Pending | Exact-head CI, live E2E, PR heads, submodule pins, and clean trees pass. Release remains pending because required G23 independent QA is not complete and G21 remains explicitly partial rather than green. |
