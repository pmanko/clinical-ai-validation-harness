...
Amendment result: **Pass** at med-agent-hub `36284dd`. The hub emits one unversioned internal
`temporal_facts` object, traces require an exact hub commit, 376 hub tests pass, 34 focused parent
...
|---|---|---|
| med-agent-hub | `36284dd` | Temporal/date-value enforcement, final grounding lifecycle, context selection traceability, network-independent unprivileged runtime health, explicit encounter evidence, product citation-scope checks, Git-owned temporal-facts provenance, claim-local In-Depth concept/direction binding, and one normalized knowledge-evidence path with deterministic multi-turn recall and complete provenance; 376 tests pass and independent re-review is clean. |
...
| Warm performance observation | Pending recollection: the previous identity-bound distribution described hub `de8614e`; the first canonical run observed a 2.507-second warm E4B `answer_done`, but G20 requires a distribution against committed hub `36284dd` and its exact labeled image. The criterion remains relative with `fixed_latency_threshold: null`. |
| Live multi-turn/preemption | Pending committed-head rerun: the focused proof on `de8614e` observed a positive Q2 cancellation marker with `router_lock_released=true` and a completed Q3 trace, but the current hub is now `36284dd`. |
...
| Remaining proof | Push the parent pin for hub `36284dd`, deploy the exact clean heads, require a complete `indepth_done` lifecycle, regenerate G09/G18-G20 and video evidence, then run, independently judge, inspect, and publish the 24-cell candidate. Final DIGI-UW/code-qa and release hygiene remain pending. |
...
| G20 Performance | In progress | The collector binds deployed image revision plus source/config/model identity; the warm distribution must be recollected on hub `36284dd` without an arbitrary absolute latency threshold. The single 2.507-second warm observation is encouraging but is not the required distribution. |
