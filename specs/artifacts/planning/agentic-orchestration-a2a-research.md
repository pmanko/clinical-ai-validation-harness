...
1. **Orchestrator = ONE agentic-loop mechanism with two orthogonal knobs**, behind the OpenAI-compat boundary and **compared empirically in the 006 validation harness**. (Corrects an earlier flawed 3-way matrix that treated "cloud" as a separate strategy — model ⊥ mechanism — and posited a separate hard-coded deterministic code path.)
   - **Mechanism:** a single ReAct-style agentic loop over the typed tool interface (KB + medical-expert) → strict JSON envelope.
   - **Knob A — guidance strength (system prompt):** the **default is STRONG guidance** that prescribes the path (search KB → consult expert → synthesize → envelope) so the loop behaves near-deterministically; relaxed guidance for open-ended / multi-hop clinical questions. This is *prompt* configuration on the one loop, **not** a separate code path.
   - **Knob B — model (swappable engine):** local gemma-4 (default) or a frontier cloud model (quality ceiling / comparison) — the cloud model **plugs into the same loop**; being reliable at multi-step tool-calling, it can safely be given more autonomy (looser guidance).
   - **Code-hardening is a deferred promotion:** turn the strong-guidance default into a hard-coded control flow ONLY if the 006 harness shows the small model can't reliably hold the guided path (the path is short, ≈1-2 hops / near single-turn — the regime small models *do* handle; reliability on our exact surface is currently unmeasured). Measure first; don't build it speculatively.
