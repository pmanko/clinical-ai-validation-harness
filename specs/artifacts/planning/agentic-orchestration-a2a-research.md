...
**REPLACE A2A as the foundation for the in-house med-agent-hub team. Standardize on a typed tool interface (MCP-for-tools deferred). Keep the existing ReAct agentic loop as the orchestrator — start there, no deterministic pipeline up front. Keep A2A only as a deferred future seam (adopt the day an agent becomes remote / cross-vendor / independently lifecycle-managed).**

> NOTE (synced with user 2026-05-29): the research's raw recommendation was "a light DETERMINISTIC workflow." We are **not** adopting that as the starting point — see the Reconciled decision below. med-agent-hub already has a flexible ReAct router/executor; that is the start. The deterministic-workflow framing is a *deferred promotion* triggered only by harness measurement, not a v1 build. Treat any "deterministic" / "near-deterministic default path" language elsewhere in this doc as superseded by §"Reconciled decision".
...
1. **Orchestrator = the existing ReAct agentic loop med-agent-hub already has — start there, add no deterministic complexity up front.** Behind the OpenAI-compat boundary, **compared empirically in the 006 validation harness**. (Corrects two earlier over-rotations: a flawed 3-way matrix that treated "cloud" as a separate strategy — model ⊥ mechanism — and, more importantly, an over-emphasis on "deterministic" / a "strong-guidance near-deterministic default path." The flexible ReAct router/executor that already exists IS the starting point; we do not rebuild it as a pipeline or hard-prescribe a fixed flow before observing the real question range.)
   - **Mechanism:** the existing ReAct-style agentic loop over the typed tool interface (KB + medical-expert) → strict JSON envelope. Kept flexible on purpose — clinical questions span a wide range, and the agent set will grow (admin, clinical-research, …); a flexible router accommodates new agents with no rewrite, a fixed pipeline would need one per agent.
   - **Knob A — guidance strength (system prompt):** *light, provisional* guidance to start. Any prescribed path (e.g. search KB → consult expert → synthesize → envelope) is a **HYPOTHESIS the harness tests against real questions, not a flow to hard-prescribe.** Tune guidance up or down based on measured results per question-type; do not assume one default path fits the range.
   - **Knob B — model (swappable engine):** local gemma-4 (default) or a frontier cloud model (quality ceiling / comparison) — the cloud model **plugs into the same loop**; being reliable at multi-step tool-calling, it can safely be given more autonomy.
   - **Deterministic control flow is a deferred promotion, not a starting point.** Introduce hard-coded control flow / a state machine ONLY if the 006 harness shows the loop can't reliably hold a behavior that matters. Measure first against the real question range; don't build it speculatively. Distinguish this **orchestration complexity** (defer) from thin **output guards** (§ guardrails doc) — the latter wrap the loop's output, are a few lines not a pipeline, and are themselves measurement-driven (not preconditions to starting).

2. **Tool layer = a clean typed tool INTERFACE now, NOT the MCP protocol yet** *(confirmed with user)*. The KB is a plain `kb.search(query) -> snippets` function (a direct API/CLI/SQL call); the medical-expert is just an OpenAI-compat call. Grounded by Anthropic "Code execution with MCP" (tool-def + intermediate-result bloat; 150k→2k tokens / 98.7% via code-over-tools; "direct tool calls remain appropriate for simpler scenarios") and Cloudflare "Code Mode" ("LLMs are better at writing code to call MCP than calling MCP directly"; MCP earns its keep at scale + uniformity). We have neither the scale nor the chaining pain points, so the **protocol is deferred**; MCP / code-mode is the seam we adopt when tools multiply or are consumed by external agents.
