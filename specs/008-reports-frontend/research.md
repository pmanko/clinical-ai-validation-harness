...
## Open producer-side note (foundational dependency)

The single harness change Decision 5 implies (exporting computed per-arm aggregates at `judge-finalize`) is small and additive, but it is foundational for durable ingest validation. Until it lands, tests may use only a documented compatibility fixture containing already-computed producer aggregate fields. The platform must not fall back to deriving benchmark or per-arm aggregate semantics from raw judge axes; the durable design ingests producer-computed numbers and keeps scoring logic out of the platform.
