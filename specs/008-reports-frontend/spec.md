...
3. **Given** a human-reviewed subset of a run, **When** the headline is presented, **Then** a calibrated estimate with its uncertainty, reviewed-cell count, reviewed subset label, and source review tier(s) is shown, distinct from the raw LLM-judge headline.
...
A researcher (or an automated agent) filters the catalog — by model/arm, comparison set, or date — to find relevant runs, and can retrieve a run's structured data (arms, per-arm scores, per-cell answers/traces, human adjudications) in a machine-readable form without scraping rendered HTML. Score filtering is deferred from v1 unless it is added explicitly to the API contract and tests.
...
- A cell adjudicated by more than one reviewer (different tiers) MUST show each review distinctly without silently overwriting, and the calibrated headline MUST make clear which reviewed subset, count of reviewed cells, and uncertainty representation it reflects.
...
- **FR-007**: The platform MUST serve score aggregations (the headline benchmark, per-arm summaries, safety counts) as values computed ONCE by the producing pipeline and exported in the producer-computed aggregate artifact (`summary.json`); the API and presentation MUST NOT re-derive or re-implement the scoring logic.
...
- **FR-014**: The store MUST be populated from the artifacts the existing validation harness produces, with one additive producer contract for the `summary.json` aggregate export; the harness remains the sole run/scoring producer and the platform consumes artifacts rather than reaching into harness internals.
...
- **FR-019**: When a human-reviewed subset of a run exists, the platform MUST present a calibrated headline estimate with its uncertainty, distinct from the raw LLM-judge headline and labeled with the reviewed subset, reviewed-cell count, and source review tier(s) it reflects.
...
- **Catalog Meta**: Singleton catalog/header prose such as the intro and scoring note, kept separate from run-derived data.
...
- **Operating metadata**: Run manifests, the run-event log, per-arm frozen configuration, reasoning traces, automated judge outputs, the producer-computed aggregate export, human adjudications, and the curated report prose — captured in the store for query and display; the originating artifacts remain the producer's outputs.
...
- **SC-002**: Each shared rendered concept (arm card, confidence treatment, reasoning trace, score table) has exactly ONE presentation definition reused by catalog, report, live, and review routes — verified by route/component tests rather than duplicated per-surface implementations (today: each is implemented twice, in two languages).
...
- **SC-004**: A reader can locate a run by model/arm, comparison set, or date and open its report in under 30 seconds, without scanning files or knowing run identifiers; for a typical stored run, catalog and report API/page loads complete in under 1 second.
...
- **SC-007**: The published per-report payload is materially smaller than today's 1–2 MB self-contained HTML, because data and presentation are no longer inlined per run; validation compares the new payload against that baseline or verifies the report no longer embeds a self-contained presentation bundle per run.
...
- **SC-011**: When a human-reviewed subset of a run exists, a calibrated headline with its uncertainty is shown distinct from the automated-judge headline and labeled with the reviewed subset, reviewed-cell count, source review tier(s), and uncertainty representation.

## Assumptions

- The existing validation harness continues to PRODUCE its run artifacts (results, automated judge outputs, traces, manifests, and human-adjudication records) as it does today, plus the additive `summary.json` aggregate export; this feature consumes/ingests those artifacts and does not take over run production (FR-014).
- Automated score aggregation logic stays with the producing pipeline; the platform displays computed values from `summary.json` rather than re-deriving them (FR-007). This boundary is what prevents re-creating today's duplication in a new layer.
- The human-review calibration approach (per-cell adjudication + a calibrated estimate with uncertainty over a reviewed subset) reuses the harness's existing review/calibration model where available. The platform's contract labels the reviewed subset, reviewed-cell count, source tier(s), estimate, and uncertainty, and does not claim calibration beyond that subset.
