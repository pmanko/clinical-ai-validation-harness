"""mod__logic_token_registration (carry-forward orphan) must transplant its datetimes.
...
logic_token_registration table has two datetime columns (date_created,
date_changed); a bare `SELECT *` or a bare `src.<col>` passthrough renders
unshifted and fails. Only the @shift_date wrapper produces the DATE_ADD/NULLIF
form. The output aliases must stay date_created / date_changed so the
carry-forward output schema is unchanged.
"""
...
MODEL_NAME = "refapp_28_demo.mod__logic_token_registration"

# The two datetime columns on legacy logic_token_registration that must be shifted.
