"""Behavior test: stg_patient_identifier_type applies the uniform date-transplant.

Renders the actual `stg_patient_identifier_type.sql` model through the real
`sqlmesh render` CLI (the production code path: macro auto-discovery + python_env
construction + macro expansion) and asserts the rendered SQL, not source strings.
Date columns must come out wrapped in `DATE_ADD(NULLIF(col, <zero>), INTERVAL
<delta> DAY)` under their original output names; FK/text/boolean columns must stay
bare.
...
def test_date_columns_are_shifted_under_original_names():
    rendered = _render_stg_patient_identifier_type()
