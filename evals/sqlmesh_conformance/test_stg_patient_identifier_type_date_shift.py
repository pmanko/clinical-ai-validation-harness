...
Renders the actual `stg_patient_identifier_type.sql` model in-process through the
real SQLMesh `Context` (`get_model().render_query_or_raise()` — macro
auto-discovery + python_env construction + macro expansion) and asserts the
rendered SQL, not source strings. This is the same render path the sibling stg_*
conformance tests use, so it needs no warehouse connection. Date columns must
...
MODEL = "refapp_28_demo.stg_patient_identifier_type"
