"""Behavior test: stg_program_workflow_state shifts its date columns via @shift_date.

The staging passthrough model `stg_program_workflow_state` carries two
date/datetime source columns — `date_created` and `date_changed`. Both must be
wrapped with the uniform date-transplant macro so they ride the same delta as
every other model, while keeping their original output names.
...
def stg_program_workflow_state_sql() -> str:
    """Rendered (lowercased, quote-stripped) SQL for stg_program_workflow_state."""
    sqlmesh = pytest.importorskip("sqlmesh")
    ctx = sqlmesh.Context(paths=str(SQLMESH_DIR))
    model = ctx.get_model("refapp_28_demo.stg_program_workflow_state")
    assert model is not None, "stg_program_workflow_state model not found in project"
    return _strip_quotes(model.render_query_or_raise().sql(dialect="mysql").lower())


def test_date_columns_are_wrapped_in_shift_date(stg_program_workflow_state_sql: str):
    sql = stg_program_workflow_state_sql
