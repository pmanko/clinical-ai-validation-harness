"""stg_concept_name_carryforward must transplant every date column via @shift_date.
...
MODEL_NAME = "refapp_28_demo.stg_concept_name_carryforward"

# The three date/datetime audit columns on legacy concept_name that must be shifted.
DATE_COLUMNS = ("date_created", "date_changed", "date_voided")
...
def test_non_date_columns_are_not_shifted():
    ...
    projections = _rendered_projections()
    for col in ("concept_name_id", "concept_id", "name", "creator", "changed_by", "voided", "uuid"):
        assert col in projections, f"output column {col} disappeared from the model"
